...
<body>
	<form id="myform" class="form-inline" action="" method="post" style="background-color: white;">
...
<!-- 		<button type="button" class="btn btn-default " id="btn_th"></i>退回</button> -->
	...
	<input type="hidden" name="year" value="${year}">
		<div id="title">
			...
			<h1 align="center">威海海洋职业学院部门收支预算编制表</h1>
		</div>
		<div id="bmmc" class="form-group" style="width: 100%;">
			<span style="margin: auto;">部门名称：
				<c:if test="${operateType=='C' }">
					<input type="text" class="form-control input-radius" name="bmmc" readonly="readonly" id="txt_bmmc" value="${bmmc }">
				</c:if>
				<c:if test="${operateType=='U' }">
					<input type="text" class="form-control input-radius" name="bmmc" readonly id="txt_bmmc" value="${info.bmmc }">
				</c:if>
			</span>
		</div>
		<br>
		<div id="ysbm" class="form-group" style="width: 100%;">
			<span style="margin: auto;">预算编码：
				<input type="text" class="form-control input-radius" name="ysbm"  id="txt_ysbm" value="${info.ysbm }">
			</span>
		</div>
		<br>
		<div class="form-group" id="bzrq_top" style="width: 100%;">
			<span style="margin: auto;">申请时间：</span>
			<input type="text" id="txt_sqsj" class="form-control date" name="sqsj" value="${info.sqsj}">
		</div>
		<div id="bottom" class="form-group" style="width: 100%;">
		<div style="margin: auto;">
			<span id="bmfzr" >部门负责人：
				<c:if test="${operateType=='C' }">
					<input type="text" class="form-control input-radius" name="bmfzr" readonly id="txt_bmfzr" value="${rymc }">
				</c:if>
				<c:if test="${operateType=='U' }">
					<input type="text" class="form-control input-radius" name="bmfzr" readonly id="txt_bmfzr" value="${info.bmfzr }">
				</c:if>
					<button type="button" id="btn_bmfzr" class="btn btn-link btn-custom">选择</button>
			</span>
			<span id="jbr" >经办人：
				<input type="text" class="form-control input-radius" name="jbr" readonly id="txt_jbr" value="${info.jbr }">
				<button type="button" id="btn_jbr" class="btn btn-link btn-custom">选择</button>
			</span>
			<span id="bzrq" >编制日期：
				<input type="text" id="txt_bzrq" class="form-control date" name="bzrq" value="${info.bzrq}" >
			</span>
		</div>
		</div>
