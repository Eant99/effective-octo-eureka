...
$(document).on("change","[id='txt_shzt']",function(){
		var shzt = $(this).val();
		table.ajax.url("${ctx}/ysgl/bmyssb1/getPageList?shzt="+shzt);
		table.ajax.reload();
	});
var shzt = "${param.shzt}";
...
$(function () {
    ...
    var columns = [
    	{"data": "GUID",orderable:false, "render": function (data, type, full, meta){
   		...
   		{"data": "ZCYSHZ",defaultContent:"-","class":"text-right"},
   	   	...
   	   	return '<a class="btn btn-link btn_upd" dwbh = "'+full.GUID+'">编辑</a>|<a  class="btn btn-link btn_delxx" guid = "'+full.GUID+'">删除</a>';
   	   	/* 	if(full.SFFH=='1'){
   	   			return '';
   	   		}else{
   	   			if(full.SHZTSZ=='0'||full.SHZTSZ=='012'){
   ...
   table = getDataTableByListHj("mydatatables","${ctx}/ysgl/bmyssb1/getPageList",[2,'asc'],columns,0,1,setGroup);
 ...
 //办理记录
  	$(document).on("click",".btn_bljl",function(){
		...
		$("#btn_add").click(function() {
			$.ajax({
				type:"post",
				url:"${ctx}/ysgl/bmyssb1/checkIsAdd",
				data:"",
				async:false,
				success:function(val){
					...
					if(result=="1"){
						$.ajax({
							type:"post",
							url:"${ctx}/ysgl/bmyssb1/checkIsAdd2",
							data:"",
							async:false,
							success:function(val){
								var result = JSON.getJson(val);
								if(result=="1"){
									alert("您所在的单位本年度已经提交预算！");
								}else{
...
//    		}else{
   			if(checkbox.length>0){
   	   		...
   	   		doOperate("${ctx}/ysgl/bmyssb1/doSubmit?dwbh="+dwbh)
...
//    		}
	...
	$(document).on("click",".btn_delxx",function(){
  		...
  		doDel("guid="+guid,"${ctx}/ysgl/bmyssb1/doDeleteAll?guid="+guid,function(val){
  			table.ajax.reload();
  		},function(val){
  		},"1");
  	});
