...
//配置iframeUlr
var _pageUrl="${pageContext.request.contextPath}/Jjkmybmdysz/jjkmList?pname=${param.pname}&controlId=${param.controlId}&type=${param.type}";
   ...
   /*  var str = "<div class='ext-tree-search input-group'><input class='form-control' type='text' id='txt_treesearch' placeholder='请输入完整功能科目名！' ><span class='input-group-btn'><button type='button' title='点击搜索信息' class='btn'><i class='faw fa-search' aria-hidden='true'></i></button></span></div>";
    ...
    $(".btn").click(function(){
 	   ...
 	   if(treesearch!=null && treesearch!=""){
 		  $("#iframe_list_${param.mkbh}").attr("src","${pageContext.request.contextPath}/Jjkmybmdysz/gojjkmPage?treesearch="+treesearch);  
 	   }else{
 		   alert("请输入检索信息！");
 	   }
    });  */
