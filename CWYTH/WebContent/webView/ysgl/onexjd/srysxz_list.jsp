...
$(function () {
    ...
    var columns = [
       ...
       {"data": "BNYSZE",defaultContent:"",'render': function (data, type, full, meta){
           return '<input type="type" disabled="disabled" display="none" name="bnysze" value="50.0000" class="text-right number1" style="width:100%;border:none;"  guid = "'+full.BNYSZE+'">';
         },"width":200},
   ...
   var i=0;
	$(document).on("blur",".checkval",function(){
   		...
   		$(this).parent().prev().find("input:first").attr("id","bnysze"+i+"");
   		$(this).parent().next().find("input:first").attr("id","ce"+i+"");
   		var bnysze=Number($("#bnysze"+i+"").val());  		
   		var val1=$(this).val();
   		if(val1.length==0 ){
   			
   		}else{
   			$("#ce"+i+"").val(bnysze-val1);
   			$("#ce"+i+"").focus();
