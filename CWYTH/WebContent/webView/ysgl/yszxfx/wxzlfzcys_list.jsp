			...
			<div class='responsive-table'>
				<div class='scrollable-area'>
					<form id="myform" class="form-inline" action="" style="padding-top: 8px;padding-bottom: 2px;">
						<center><h4><strong>维修租赁费支出预算汇总表</strong></h4></center>
						<div class="search-simple">	
							<!-- <div class="form-group"> 
								<label>填报部门</label>					
...
// 		$("#txt_dwbh").bindChange("${ctx}/suggest/getXx","D");
	...
	console.log("+++++++++++运行了");
	table = getDataTableByListHj("mydatatables","${ctx}/yszxfx2/getPageList3",[2,'asc'],columns,0,1,setGroup);
	//双击事件
	$("tbody tr").bind("dblclick",function(){
		//var xmmc = $(this).text();
		var xmmc=$(this).children('td').eq(2).text();
		console.log("----------"+xmmc);
		doOperate("${ctx}/yszxfx2/gowxzlfzcysListPage?xmmc="+xmmc);
	});
	//打印
	$(document).on("click","#btn_print",function(){
   		doOperate("${ctx}/yszxfx2/WxzlPrint");
	});
	//导出Excel
   	$("#btn_exp").click(function(){
		...
		checkbox.each(function(){
   		...
   		doExp(json,"${ctx}/yszxfx2/expExcel3?guid="+guid.join("','"),"维修租赁费支出预算表","${pageContext.request.contextPath}",guid.join(","));
   	});
