...
$(function (){
    table = $('#mydataTables').DataTable({
        ajax: {
            ...
            url: "${pageContext.request.contextPath}/xmlx/getkjkmPageList?treedwbh=${dwbh}&kmnd=${kmnd}&dm=${dm}&jb=${jb}"//获取数据的方法
        },
	    ...
	    $("#btn_submit").click(function(){
	    	...
	    	if(checkbox.length==0){
	        	...
	        	if("${param.lx}"=="sr"){
	        		...
	        		getIframWindow("${param.pname}").addsrkm(kmbh,kmmc);
	        	}else if("${param.lx}"=="zc"){
	        		getIframWindow("${param.pname}").addzckm(kmbh,kmmc);
	        	}
	    ...
	    $(document).on("dblclick","#mydataTables tr:not(#header)",function(){
	    	...
	    	if(checkbox.length==0){
	        	...
	        	if("${param.lx}"=="sr"){
	        		...
	        		getIframWindow("${param.pname}").addzckm(kmbh,kmmc);
	        	}else if("${param.lx}"=="sr2"){
	        		getIframWindow("${param.pname}").addsrkm2(kmbh,kmmc);
	        	}else{
	        		getIframeControl("${param.pname}","${param.controlId}").val(kmbh);
	    			getIframeControl("${param.pname}","${param.controlId1}").val(kmmc);
...
// 	        		getIframWindow("${param.pname}").addBhMcToSz(kmbh,kmmc);
	        	}
	        	close(winId);
	        	}
	    	}
