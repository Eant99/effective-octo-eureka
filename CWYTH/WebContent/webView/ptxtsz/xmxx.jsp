...
$(function () {
    ...
    var columns = [
       {"data": "GUID",orderable:false, 'render': function (data, type, full, meta){
         return '<input type="checkbox" name="guid" class="keyId" data-xmbh="'+full.XMBH+'" data-xmmc="'+full.XMMC+'" value="' + data + '" values="' +"("+ full.BMBHMC + ")"+full.XMMC+'" bmbh="'+full.BMBH+'" >';
    ...
    $(document).on("dblclick","#mydatatables tr:not(#header)",function(){
    	...
    	//获取项目编号
    	var val2=$(this).find("[name='guid']").attr("data-xmbh");
    	//获取部门编号
    	var val3=$(this).find("[name='guid']").attr("bmbh");
    	//项目名称
    	var val4=$(this).find("[name='guid']").attr("data-xmmc");
    	console.log(val2+val3);
    	if(val==''||val==null){
    		alert("没有可以选择的数据！");
    	}else{
    		getIframeControl("${param.pname}","${param.controlId}").val(val);
    		getIframeControl("${param.pname}","${param.controlId1}").val(val2);
    		getIframeControl("${param.pname}","${param.controlId2}").val(val3);
    		getIframeControl("${param.pname}","${param.controlId3}").val(val4);
  	 ...
  	 $(document).on("click","#btn_qd",function(){
  		 ...
  		 if(checkbox.length==0){
  			...
  			//获取项目编号加信息
  	    	var val = checkbox.attr("values");
  	    	//获取项目编号
  	    	var val2=checkbox.attr("data-xmbh");
  	    	//获取部门编号
  	    	var val3=checkbox.attr("bmbh");
  			getIframeControl("${param.pname}","${param.controlId}").val(val);
  			getIframeControl("${param.pname}","${param.controlId1}").val(val2);
    		getIframeControl("${param.pname}","${param.controlId2}").val(val3);
