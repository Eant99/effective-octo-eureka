...
//配置iframeUlr
var _pageUrl="${pageContext.request.contextPath}/cbys/goPageList";
//权限标志
var _type="self";
Ext.onReady(function(){
	var root = new Ext.tree.AsyncTreeNode({
        ...
        loader: new Ext.tree.TreeLoader({
        	 url: "${pageContext.request.contextPath}/cbys/CbysTree",
             listeners: {
...
//     }); 
   // $("#txt_treesearch").bindChange("${ctx}/suggest/getXx","ZD");
     ...
     * 左侧树的联想查询
     */
    	var str = "<div class='ext-tree-search input-group'><input class='form-control' type='text' id='txt_treesearch' placeholder='请输入要素编号或要素名称！' ><span class='input-group-btn'><button type='button' title='点击搜索信息' class='btn'><i class='faw fa-search' aria-hidden='true'></i></button></span></div>";
        $("#left_tree .x-panel-body").prepend(str);
        $(".btn").click(function(){
     	   var treesearch = $("#txt_treesearch").val();
     	   if(treesearch!=null && treesearch!=""){
     		  $("#iframe_list_${param.mkbh}").attr("src","${pageContext.request.contextPath}/cbys/goPageList?treesearch="+treesearch);  
     	   }else{
     		   alert("请输入检索信息！");
     	   }
        }); 
