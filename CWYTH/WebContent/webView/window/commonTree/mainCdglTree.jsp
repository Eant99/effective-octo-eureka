...
//配置iframeUlr
var _pageUrl="${pageContext.request.contextPath}/cdgl/goCdglList";
// var _pageUrl="${pageContext.request.contextPath}/cdgl/goCdglList?mkbh=${param.mkbh}";
//权限标志
var _type="self";
...
//                             mkbh: "root",
     ...
     * 左侧树的联想查询
     */
    	var str = "<div class='ext-tree-search input-group'><input class='form-control' type='text' id='txt_treesearch' placeholder='请输入功能菜单编号或名称！' ><span class='input-group-btn'><button type='button' title='点击搜索信息' class='btn'><i class='faw fa-search' aria-hidden='true'></i></button></span></div>";
        $("#left_tree .x-panel-body").prepend(str);
        $(".btn").click(function(){
     	   var treesearch = $("#txt_treesearch").val();
     	   if(treesearch!=null && treesearch!=""){
     		  $("#iframe_list_${param.mkbh}").attr("src","${pageContext.request.contextPath}/cdgl/goCdglList?treesearch="+treesearch);  
     	   }else{
     		   alert("请输入检索信息！");
     	   }
        }); 
        $("#txt_treesearch").bindChange("${pageContext.request.contextPath}/suggest/getXx","GNCD");
