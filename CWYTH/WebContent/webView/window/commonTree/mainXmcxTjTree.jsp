...
//配置iframeUlr
var _pageUrl="${pageContext.request.contextPath}/"+"${pageUrl}";
//权限标志
var _type="self";
Ext.onReady(function(){
	var root = new Ext.tree.AsyncTreeNode({
        ...
        loader: new Ext.tree.TreeLoader({
	        //单位权限
	        url: "${ctx}/xmgl/xmcx/xmchaxun/tjxmcxTree",
	        listeners: {
     ...
     * 左侧树的联想查询
     */
    var str = "<div class='ext-tree-search input-group'><input class='form-control' type='text' id='txt_treesearch' placeholder='输入人员工号或姓名' ><span class='input-group-btn'><button type='button' title='点击搜索信息' class='btn'><i class='faw fa-search' aria-hidden='true'></i></button></span></div>";
        $("#left_tree .x-panel-body").prepend(str);
        $(".btn").click(function(){
     	   var treesearch = $("#txt_treesearch").val();
     	   if(treesearch!=null && treesearch!=""){
     		  $("#iframe_list_${param.mkbh}").attr("src","${ctx}/xmgl/xmcx/xmchaxun/goXmcxPage?treesearch="+treesearch);  
     		  console.log("treesearch==="+treesearch);
     	   }else{
     		   alert("请输入检索信息！");
     	   }
        }); 
    $("#txt_treesearch").bindChange("${ctx}/suggest/getXx","R");
});
</script>
</body>
</html>