...
$(function (){
    table = $('#mydataTables').DataTable({
        ajax: {
            url: "${pageContext.request.contextPath}/window/ryxx?treesearch=${param.treesearch}&dwbh=${param.dwbh}&flag=${param.flag}&mkbh=${param.mkbh}"//获取数据的方法
        },
        ...
        "columns": [
            {"data": "RYGH",orderable:false, 'render': function (data, type, full, meta){
	             return "<input type='checkbox' class='keyId' guid='"+full.GUID+"' name='rygh' xm='"+full.XM+"' rybh='"+full.RYBH+"' dwmc='"+full.DWMC+"' lxdh='"+(full.LXDH==undefined?'':full.LXDH)+"' value='("+data+")"+full.XM+"'>";
    ...
    $(document).on("dblclick","#mydataTables tr:not(#header)",function(){
    	...
    	var xm = $(this).find("[name='rygh']").attr("xm");
    	var rydom = $(this).find("[name='rygh']");
    	if(guid==''||guid==null||guid=='undefined'){
    		alert("没有可以选择的数据！");
    	}else{
    		if("040101"=="${param.mkbh}"){
    			getIframeControl("${param.pname}","${param.controlId1}").val(rybh);
    			getIframeControl("${param.pname}","${param.controlId2}").val(xm);
    			getIframeControl("${param.pname}","${param.controlId1}").focus();//手动触发验证
    			getIframeControl("${param.pname}","${param.controlId1}").trigger("blur");//手动触发验证
    		}else{
   				if(flag=="pzfz"){
   	    		  getIframeControlForPzfz("${param.pname}","${param.controlId}").val(ry);
   	    		}else{
   	    		...
   	    		if("${param.controlId2}" !=''){
   	    			getIframeControl("${param.pname}","${param.controlId2}").val(dwmc);
   	    		}
   	    		if("${param.controlId3}" !=''){
   	    			getIframeControl("${param.pname}","${param.controlId3}").val(dwmc);
   	    		}
   	    		if("${param.szbmm}" !=''){
   	    		...
   	    		if(flag=="pzfz"){
   	      		  getIframeControlForPzfz("${param.pname}","${param.controlId}").focus();
   	      		}else{
