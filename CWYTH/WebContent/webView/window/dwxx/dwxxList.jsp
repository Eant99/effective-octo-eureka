...
$(function (){
	...
	var columns = [
	           	{"data": "DWBH",orderable:false, "render": function (data, type, full, meta){
	        		return '<input type="checkbox" class="keyId" name="dwbh" bm="'+full.BMH+'" bmmc="'+full.MC+'" value="' + "("+full.BMH+")"+full.MC + '">';
    ...
    $(document).on("dblclick","#mydatatables tr:not(#header)",function(){
    	if("040101"=="${param.mkbh}"){
	    	var bm = $(this).find("[name='dwbh']").attr("bm");
	    	var bmmc = $(this).find("[name='dwbh']").attr("bmmc");
    		getIframeControl("${param.pname}","${param.controlId1}").val(bm);
    		getIframeControl("${param.pname}","${param.controlId2}").val(bmmc);
