...
<form id="myform" class="form-horizontal" action="" method="post">
	<input type="hidden" name="guid"  value="${zyxx.GUID}">
				...
				<div class="row">
					<div class="col-md-4">
						<div class="input-group">
							...
							<input type="text" id="txt_zybh" name="zybh" class="form-control input-radius"  value="${zyxx.ZYBH}"/>
						</div>
                    </div>
                    <div class="col-md-4">
						<div class="input-group">
							<span class="input-group-addon"><span class="required">*</span>专业名称</span>
                            <input type="text" id="txt_zymc" class="form-control input-radius" name="zymc" value="${zyxx.ZYMC}"/>
						</div>
					</div>
					<div class="col-md-4">
						<div class="input-group">
							<span class="input-group-addon"><span class="required">*</span>所属院系</span>
							<input type="text" id="txt_ssyx" class="form-control input-radius window" name="ssyx" value="${zyxx.SSYX}">
...
<%-- 	                         <c:if test="${operateType == 'C' }"> --%>
		                        <input type="text" id="txt_zyfx" class="form-control input-radius window" name="zyfx" value="${zyxx.ZYFX}">
...
<%-- 	                         	<input type="text" id="txt_zyfx1" class="form-control input-radius window" name="zyfx" readonly="readonly" value="${zyxx.ZYFX}"> --%>
<%-- 	                         </c:if> --%>
						</div>
					</div>
					<div class="col-md-4">
						<div class="input-group">
	                       ...
	                       <select id="drp_xz" class="form-control input-radius select2" name="xz">
	                                <c:forEach var="xzlist" items="${xzList}"> 
	                                   <option value="${xzlist.DM}" <c:if test="${zyxx.XZ == xzlist.DM}">selected</c:if>>${xzlist.MC}</option>
									</c:forEach>
	                            </select>
						</div>
					</div>
					<div class="col-md-4">
						...
						<div class="pull-left" style="padding-left: 10px;">
							<input type="radio" name="zyzt" value="0" <c:if test="${zyxx.ZYZT==0}">checked</c:if>/>禁&emsp;&emsp;用&emsp;&emsp;
							<input type="radio" name="zyzt" value="1" <c:if test="${zyxx.ZYZT==1}">checked</c:if>/>启&emsp;&emsp;用
						</div>
						</div>
						</div>
					</div>
...
$(function(){
	...
	$("#btn_save").click(function(){
		doSave(validate,"myform","${ctx}/zyxxwh/doSaveEdit",function(val){
			if(val.success){
				alert(val.msg);
