...
$(function (){
	...
	var columns = [
	           	{"data": "FLBH",orderable:false, "render": function (data, type, full, meta){
	        		return '<input type="checkbox" class="keyId" name="FLBH" value="' + "("+full.FLBH+")"+full.FLMC + '">';
				},"width":10,'searchable': false},
	            {"data": "FLBH",defaultContent:"","render": function (data, type, full, meta){
	...
	table = $('#mydatatables').DataTable({
        ajax: {
            url: "${pageContext.request.contextPath}/xmsb/xmsb/xmflPage"//获取数据的方法
        },
        ...
        "language":{
            "search":"",
            "searchPlaceholder":"请输入项目分类信息关键字"
        },
    ...
    $(document).on("dblclick","#mydatatables tr:not(#header)",function(){
    	var val = $(this).find("[name='FLBH']").val();
    	var val1 = $(this).find("[name='dm']").val();
    	if(val==''||val==null||val=='undefined'){
    		alert("没有可以选择的数据！");
    	}else{
    		getIframeControl("${param.pname}","${param.controlId}").val(val);
    		getIframeControl("${param.pname}","txt_xmbh").val(val1);
