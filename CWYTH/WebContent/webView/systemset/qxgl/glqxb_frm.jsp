...
<title>管理权限设置单位树</title>
﻿<link href="${ctxStatic}/css/bootstrap/bootstrap.min.css" rel="stylesheet" type="text/css" />
<link href="${ctxStatic}/css/public/googosoft.css" rel="stylesheet" type="text/css" />
<style type="text/css">
	.input-group {
		padding: 5px;
	}
...
//配置iframeUlr
var _pageUrl="${ctx}/glqxb/getGlqxszList";
//权限标志
var _type="self";
Ext.onReady(function(){
    ...
    var right = new Ext.Panel({
         ...
         title:"",
         autoScroll:true,
         html: "<iframe frameborder='0' width='100%' height='100%' scrolling='auto' id='iframe_list_${param.mkbh}' name='iframe_list_${param.mkbh}' src='" + _pageUrl + "?mkbh=${param.mkbh}&rybh=<%=LUser.getRybh()%>'></iframe>",
         border: true
    });
	/*创建窗体分割器*/
    new Ext.Viewport({
   		   ...
   		   $.ajax({
				...
				success:function(result){
					if(result.trim() != "" && result.trim() != "F"){
						$("#iframe_list_${param.mkbh}").attr("src","${ctx}/glqxb/getGlqxszList?rybh="+result);
