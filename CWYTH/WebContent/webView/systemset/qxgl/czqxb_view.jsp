										...
										<td rowSpan="${menu_ej.size}" class="col-md-2">${menu_ej.MKMC}</td>
		                           		<c:forEach var="menu_sj" items="${menu_ej.childList}">
		                           			<c:if test="${menu_sj.idx > 0}"><tr></c:if>
												<td class="col-md-3"><div><a href="javascript:void(0);" class="btn btn-lookl btn-link">${menu_sj.MKMC}<input type="hidden" value="${menu_sj.MKBH}"></a></div></td>											
	                            			</tr>
										</c:forEach>
									</c:forEach>
								</tr>
	...
	$(function(){
		...
		//查看
	   	$(document).on("click",".btn-lookl",function(){
	   		var mkbh = $(this).find(":hidden").val();
	   		
	   		select_commonWin("${ctx}/czqxb/getMkxx?mkbh="+mkbh,"有此模块操作权限的人员信息", "400", "450");
	   	});
		/*左侧列表全选复选框按钮*/
