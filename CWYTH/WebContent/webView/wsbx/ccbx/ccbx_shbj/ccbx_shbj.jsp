        	...
        	<table id="mydatatables" class="table table-striped table-bordered" >
			    	...
			    	<c:forEach items="${fpList}" var="fpMap" varStatus="i">
						<tr>
							...
							<td style="text-align:center;vertical-align:middle;"/><span>发票号</span></td>
							<td><input type="text" id="fpha_${i.index+1}" class="input-radius sz" name="fpha" value="${fpMap.fph1}" 
									onkeyup="value=value.replace(/[^0-9.(\d{2})?$]/g,'');"/></td>
							<td><input type="text" id="fphb_${i.index+1}" class="input-radius sz" name="fphb" value="${fpMap.fph2}" 
									onkeyup="value=value.replace(/[^0-9.(\d{2})?$]/g,'');"/></td>
							<td><input type="text" id="fphc_${i.index+1}" class="input-radius sz" name="fphc" value="${fpMap.fph3}" 
									onkeyup="value=value.replace(/[^0-9.(\d{2})?$]/g,'');"/></td>
							<td><input type="text" id="fphd_${i.index+1}" class="input-radius sz" name="fphd" value="${fpMap.fph4}" 
									onkeyup="value=value.replace(/[^0-9.(\d{2})?$]/g,'');"/></td>
							<td><input type="text" id="fphe_${i.index+1}" class="input-radius sz" name="fphe" value="${fpMap.fph5}" 
									onkeyup="value=value.replace(/[^0-9.(\d{2})?$]/g,'');"/></td>
						</tr>
					</c:forEach>
				    </tbody>
							...
							<div class="row">
								...
								<div class="col-md-3">
									<div class="input-group">
										...
										<div id="editor" class="border-bottom: 1px solid #ccc;">
											<input type="text" id="kssj_1" name="kssj" class="form-control input-radius datetime" value=""/>
										</div>
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
										...
										<div id="editor" class="border-bottom: 1px solid #ccc;">
											<input type="text" id="jssj_1" class="form-control input-radius datetime" name="jssj" value=""/>
										</div>
									</div>
								</div>
							</div>
							...
							<div class="row">
								<div class="col-md-3">
									<div class="input-group">
										...
										<div id="editor" class="border-bottom: 1px solid #ccc;">
											<input type="text" id="jsbzbz_1" name="jsbzbz" class="number null form-control bzbz" readonly
												value="" onkeyup="value=value.replace(/[^0-9.(\d{2})?$]/g,'');"/>
										</div>
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
										...
										<div id="editor" class="border-bottom: 1px solid #ccc;">
											<input type="text" id="snjtbzbz_1" name="snjtbzbz" class="number null form-control bzbz" readonly
												value="" onkeyup="value=value.replace(/[^0-9.(\d{2})?$]/g,'');"/>
										</div>
									</div>
								</div>
								<div class="col-md-3">
									<div class="input-group">
										...
										<div id="editor" class="border-bottom: 1px solid #ccc;">
											<input type="text" id="zsfbz_1" name="zsfbz" class="number null form-control bzbz" readonly
												value="" onkeyup="value=value.replace(/[^0-9.(\d{2})?$]/g,'');"/>
										</div>
									</div>
								</div>
...
//  		}
	...
	function yzbxbz(){
		...
		if(rylx=='教师'){
			...
			zsfje.each(function(){
				...
				console.log("je==="+zsfje_);
				console.log("bz==="+zsfbz_);
				console.log("ts==="+ccts_);
				console.log(zsfje_>zsfbz_*ccts_);
		...
		bcbxje.each(function(){
			var syje = $(this).parents("tr").find("[name=syje]").val();//剩余金额
			if(parseFloat($(this).val())>parseFloat(syje)){
				alert("本次报销金额超出剩余金额");
				flag = false;
			}
		});
		var bcbxje=$("[name='bcbxje']");//本次报销金额
		var money=0;
		bcbxje.each(function() {
			money= money+parseFloat($(this).val());
			var syje = $(this).parents("tr").find("[name='syje']").val();
			if(parseFloat(money)>parseFloat(syje)){
				alert("本次报销金额不可大于项目剩余金额,剩余金额为"+syje+"元");
				flag = false;
			}
		});
		if(money<0 || money==0){
	...
	$("#btn_next").click(function(){
		...
		if(flag){
			doSave1(validate,"myform","${ctx}/wsbx/ccbx/doSaveZb2?type="+type,"next",function(val){
			});
		}
...
// 	   				console.log(jsonStr);
	...
	function next(type){
		...
		$.ajax({
   			...
   			success:function(data){
   				...
   				var jsonStr = JSON.stringify(json);  //json字符串
   				$.ajax({
		   			url:"${ctx}/wsbx/ccbx/doSaveXmMx?ccywguid=${ccywguid}&zbguid=${clfbxzb.guid}",
		   			type:"post",
		   			data:"json="+jsonStr,
		   			success:function(data){
		   				next2(type);  	   					
	...
	function next2(type){
		...
		$.ajax({
	   			...
	   			success:function(data){
		   			if(type=='next'){
		   				location.href = "${ctx}/wsbx/ccbx/jsfs2?money=${money}&mkbh=${param.mkbh}&zbguid=${clfbxzb.guid}&xmguid="+"${clfbxzb.xmguid}"+"&xmmc="+"${clfbxzb.xmmc}"+"&bxzje="+$("#txt_bxzje").val()+"&djbh="+$("#txt_djbh").val()+"&look="+"${look}&ccywguid=${ccywguid}&procinstid=${procinstid}";						
		   			}else if(type=='after'){
		   				location.href = "${ctx}/webView/wsbx/ccbx/xzxm.jsp?mkbh=${param.mkbh}&xmguid="+$("#txt_xmguid").val()+"&zbguid=${clfbxzb.guid}&look="+"${look}&ccywguid=${ccywguid}";
		   			}	
	   			}
		}); 
	}
