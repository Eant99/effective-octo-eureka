...
$(function () {
    ...
    var columns = [
       ...
       {"data": "",'render':function(data, type, full, meta){
    	   return '<a href="javascript:void(0);" class="btn btn-link btn_upd" guid="'+full.GUID+'"  ccywguid="'+full.CCYWGUID+'" djbh="'+full.DJBH+'">编辑</a>|<a href="javascript:void(0);" class="btn btn-link btn_look" guid="'+full.GUID+'"  ccywguid="'+full.CCYWGUID+'" >查看</a>|<a href="javascript:void(0);" class="btn btn-link btn_sub" guid="'+full.GUID+'">复核</a>|<a href="javascript:void(0);" class="btn btn-link btn_del" guid="'+full.GUID+'">删除</a>';	   		
      },orderable:false,"class":"text-center"}
     ];
  table = getDataTableByListHj("mydatatables","${ctx}/bxpzth/getclfbxPageList?mkbh=${mkbh}",[2,'desc'],columns,0,1,setGroup);
	//查看按钮
	$(document).on("click",".btn_look",function(){
 		var guid = $(this).attr("guid");
 		var ccywguid = $(this).attr("ccywguid");
 		location.href = "${ctx}/bxpzth/goClfbxEdit?mkbh=${mkbh}&guid="+guid+"&operateType=L&ccywguid="+ccywguid;
 	});
 	//修改按钮
 	$(document).on("click",".btn_upd",function(){
	   	var guid = $(this).attr('guid');
	   	var ccywguid = $(this).attr("ccywguid");
	   	location.href = "${ctx}/bxpzth/goClfbxEdit?mkbh=${mkbh}&guid="+guid+"&operateType=U&ccywguid="+ccywguid;
 	});
 	//单条删除
	$(document).on("click",".btn_del",function(){
		var guid = $(this).attr("guid");
		doDel("guid="+guid,"${pageContext.request.contextPath}/bxpzth/doDelete?type=clfbx",function(val){
			table.ajax.reload();
		},function(val){
