...
$(function () {
	var columns = [
	       	...
	       	//表数据
          	table = getDataTableByListHj("mydatatables","${ctx}/zkylbx/getPageList?treedwbh=${dwbh}",[4,'asc'],columns);
...
//          		var guid = $(this).parents("tr").find("[name='guid']").val(); 
         		...
         		var lx = $(this).attr("lx");
         		console.log("___"+guid+"__${dwbh}________"+lx);
           		doOperate("${ctx}/zkylbx/singlePrint?guid="+guid+"&dm=${dwbh}&lx="+lx);//PrintSamplerc
        	});
        	//批量打印
           	$("#btn_printpl").click(function(){
           		...
           		var djdw = $("#mydatatables").find(".djdw").filter(":checked");
           			if(checkbox.length>0){
           	   			...
           	   			console.log("__"+dwbh);
        				doOperate("${ctx}/kylbx/demoPrint?length="+checkbox.length+"&dwbh="+dwbh);
           	   		}else{
           	   			alert("请选择至少一条信息打印！");
           	   		}
...
// 			id.push($(this).val());
// 		});
//    		doExp(json,"${ctx}/zkylbx/expExcel?treedwbh=${dwbh}&id="+id.join("','"),"科研类报销汇总","${pageContext.request.contextPath}",id.join("','"));
//    	});
  ...
  //jsp 页面导出Excel
   	...
   	var checkbox = $("#mydatatables").find("[name='keyId']").filter(":checked");
		...
		doExp("","${ctx}/zkylbx/expExcel5?treedwbh=${dwbh}","出差业务申请","${pageContext.request.contextPath}",id);
...
// 					$.ajax({
// 						type : "post",
// 						data : {guid:guid.join(","),treedwbh:treedwbh},
// 						url : "${ctx}/zkylbx/expExcel5",
// 						success : function(val) {
// 						   FileDownload("${ctx}/file/fileDownload","科研类报销汇总.xls",val.url);
// 						}
// 					});
// 				} else {
// 					alert("请至少选择一条信息导出!");
