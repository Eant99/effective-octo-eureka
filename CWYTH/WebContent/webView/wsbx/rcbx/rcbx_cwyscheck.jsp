				...
				<div class="row">
					<div class="col-md-12">
						<div class="input-group">
							...
							<textarea style="width:100%;height:60px;" name="bxsy" class="form-control input-radius">${zbMap.bxsy}</textarea>
						</div>
					</div>
				</div>
<!-- 				<div class="row"> -->
				...
				<table id="mydatatables" class="table table-striped table-bordered">
		        	<thead id="thead">
				          <tr>
				            ...
				            <th style="width:200px;">报销金额（元）</th>
				            <th style="width:200px;">票据张数（张）</th>
...
<!-- 							<td class="btn_td"><div class="deleteBtn"></div></td> -->
							<tr class="tr_${i.index+1}">
								...
								<input type="hidden" id="dxt_xmid${i.index+1}"  class="xmguid" name="xmguid" value="${list.guid}">
								...
								<td>
								<div class="input-group">
								<input type="text" id="txt_fymc${i.index+1}"  style="background-color: white !important;" name="fymc" readonly class=" input-radius window null" value="${list.fymc}">
								<input type="hidden" id="txt_fyid${i.index+1}" class="fyid" name="fyid" value="${list.fyid}">
								<span class="input-group-btn">
								...
								<td>
								<input type="text" id="txt_bxje${i.index+1}" class=" input-radius sz number null" name="bxje" readonly value="${list.bxje}"
									onkeyup="value=value.replace(/[^0-9.(\d{2})?$]/g,'');">
								</td>
								<td>
								<input type="text" id="txt_pjzs${i.index+1}" class=" input-radius sz null int" name="fjzs" readonly value="${list.fjzs}"
										  onkeyup="value=value.replace(/[^0-9.(\d{2})?$]/g,'');">
								</td>
								<td>
								<input type="hidden"  name="zbid" value="${zbid}">
								<input type="text" id="txt_bz${i.index+1}" class=" null window input-radius" name="bz" readonly value="${list.bz}">
								</td>
							</tr>
				    		</c:if>
				    	</c:forEach>
				...
				<table id="mydatatables" class="table table-striped table-bordered">
				    ...
				    <tbody id="tb_cxjk">
				    	<c:forEach items="${cxList}" var="cjkList" varStatus="cjk">
							...
							<td class="btn_td">${cjk.index+1}</td>
							<td>
				...
				<table id="mydatatables" class="table table-striped table-bordered">
				    ...
				    <tbody id="tb_xnzz">
				    	<c:forEach items="${dgList}" var="dgList" varStatus="dg">
							...
							<td>
								<input type="text" id="txt_dwmc${dg.index+1}" class="bk input-radius" readonly  value="${dgList.dwmc}">
							</td>
							<td>
							...
							<td>
							<input type="text" id="txt_je${dg.index+1}" name="je" class="bk input-radius number" readonly  value="${dgList.je}">
							</td>
						</tr>
				    	</c:forEach>
				...
				<table id="mydatatables" class="table table-striped table-bordered">
				    ...
				    <tbody id="tb_sszk">
				    	...
				    	<c:forEach items="${dsList}" var="dsList" varStatus="ds">
							...
							<td>
								<input style="width:100% !important;" type="text" readonly <c:if test="${dsList.ryxz=='0'}">value="个人"</c:if> 
				    		...
				    		<td>
<!-- 				    		<div class="input-group"> -->
				    			<input type="text" style="width:100% !important;" readonly class="bk input-radius" value="${dsList.ryxm}">
<!-- 				    		</div> -->
							</td>
							<td>
				...
				<table id="mydatatables" class="table table-striped table-bordered">
				    ...
				    <tbody id="tb_gwk">
				    	<c:forEach items="${gwkList}" var="gwkList" varStatus="gwk">
							...
							<td class="btn_td" style="vertical-align: middle;">${gwk.index+1}</td>
							<td>
...
$("#btn_tg").click(function(){
	...
	var bxsy = $("[name='bxsy']").val();
	var fyid = $("[class='fyid']");
	var xmguid = $("[class='xmguid']");
	var fyids=[];
	var xmguids=[];
	$.each(fyid,function(){
		fyids.push($(this).val());
	});
	$.each(xmguid,function(){
	...
	select_commonWin("${ctx}/wsbx/rcbx/check?type=first&guid="+guid+"&procinstid="+procinstid+"&flag="+flag+"&bxsy="+bxsy+"&fyid="+fyids.join(",")+"&fcwys=${fcwys}"+"&xmguid="+xmguids.join(","),"通过页面","500","300");
...
$("#btn_th").click(function(){
	...
	select_commonWin("${ctx}/wsbx/rcbx/check?type=second&guid="+guid+"&procinstid="+procinstid+"&bxsy="+bxsy+"&fyid="+fyids.join(",")+"&fcwys=${fcwys}"+"&xmguid="+xmguids.join(","),"退回页面","500","300");
