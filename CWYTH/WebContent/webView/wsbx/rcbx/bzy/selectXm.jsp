	...
	<div class="search" id="searchBox" style="padding-top: 0px">
		...
		<form id="myform" class="form-inline" action="" style="padding-top: 8px;padding-bottom: 2px;">
			...
			<div class="pull-right" style="margin-top:3px;">
				...
				<div class="sub-title pull-left">费用修改&nbsp;></div>
				<div class="sub-title pull-left text-primary">
					<div style="width:26px; height:26px; background-color:#CCCCCC; border-radius:13px;">
...
// 	operateYwsm("${ctx}");
    ...
    var columns = [
			{"data": "GUID",orderable:false, 'render': function (data, type, full, meta){
    			...
    			return '<input type="checkbox" name="guid" class="keyId" YE="'+full.YE+'" XMMC="'+full.XMMC+'" XMLBMC="'+full.XMLBMC+'" value="' + data + '"  xmguid="'+data+'"  guid = "'+full.XMBH+'" jflxdm="'+full.JFLXDM+'">';
  			},"width":10,'searchable': false},
       {"data": "XMBH",defaultContent:"","width":40},
       {"data": "XMMC",defaultContent:"","width":1000},
       {"data": "JFLX",defaultContent:"","width":280},
  	...
  	table = getDataTableByListHj("mydatatables","${ctx}/wsbx/rcbx/getPageListByBzy?treedwbh=${dwbh}&bxr=${param.bxr}",[6,'DESC'],columns,0,1,setGroup);
});
$(function() {
	//列表右侧悬浮按钮
...
$(document).on("click","[id='btn_next']",function(){
	...
	var type = checkbox.parents("tr").find("td").eq(5).text();
	if(type.indexOf("科研")>0){
		alert("本次不能选择科研类项目！");
		return;
	}
	...
	if(checkbox.length==1){
   		...
   		if(flag==false){
	   		...
	   		var src = "${ctx}/wsbx/rcbx/ywbl?look=${param.look}&bjzt=${param.bjzt}&zbid=${param.zbid}&xmmc="+xmmcs+"&xmguid="+xmguids+"&mkbh=${param.mkbh}&djbh=${param.djbh}&money="+moneys+"&jflxdm="+jflxdms+"&bz=delete&flag="+flag;
	   		location.href = src;
   		}
	}else{
		alert("请选择一条项目信息进行操作!");
