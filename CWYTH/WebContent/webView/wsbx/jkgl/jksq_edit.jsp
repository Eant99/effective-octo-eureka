				...
				<div class="row">
					...
					<div class="col-md-4">
						<div class="input-group">
							...
							<input type="hidden" id="txt_jkr1" class="form-control input-radius" name="jkr" value="${jkr}"/>
							</c:if>
				...
				<div class="row">
					...
					<div class="col-md-4">
                    	<div class="input-group">
							...
							<input type="text" id="txt_fjzs" name="fjzs" class="form-control input-radius2 int" value="${jksq.FJZS}"  />
                        </div>
					</div>
					<div class="col-md-4">
					<div class="input-group">
	          ...
	          <div class="container-fluid box-content sszk-content">
				    ...
				    <tbody id="tb_sszk">
						...
						<tr class="tr_0">
							...
							<td>
								<select id="txt_klx0" name="klx"  class="bk input-radius"></select>
							</td>
							<td>
							<input type="text" id="txt_dfzh0" name="dsdfzh" class="bk input-radius null" readonly="readonly" value="">
							<input type="hidden" id="txt_guid0" name="zhbguid" class="bk input-radius null" readonly="readonly" value="">
							</td>
							<td>
							<input type="text" id="txt_dsje0" name="dsje" class="bk input-radius number text-right"  value="" onkeyup="value=value.replace(/[^0-9.(\d{2})?$]/g,'');">
								<input type="hidden" name="end" />
							</td>
						</tr>
				...
				<table id="mydatatables" class="table table-striped table-bordered" style="border-collapse: collapse;">
				    ...
				    <tbody id="tb_xnzz">
						...
						<tr class="tr_0">
							...
							<td>
							<input type="text" id="txt_dfzh0" name="dgdfzh" style="background-color: white !important; " readonly class="bk input-radius"  value="">
							</td>
							<td>
							<input type="text" id="txt_je0" name="dgje" class="bk input-radius text-right number "  value="">
							<input type="hidden" name="end1" value="" />
							</td>
   ...
   //返回按钮
	...
	$(document).on("click",".btn-ktmc",function(){
	 ...
	 function doSave1(_validate,$form, _url, _success, _fail){
		 	var valid;
		 	var validator = $('#myform').bootstrapValidator(_validate);
			if(validator){
	 ...
	 $("#btn_save").click(function(){	
			...
			var bxzjes = 0;
			if($('#addDg').filter(":checked").length>0){
				$("[name='dgje']").each(function(){
					bxzjes += Number($(this).val());
				});
			}
			if($('#addDs').filter(":checked").length>0){
				$("[name='dsje']").each(function(){
					bxzjes += Number($(this).val());
				});
			}
			if($('#addDg').filter(":checked").length == 0&&$('#addDs').filter(":checked").length==0){
				alert("请选择支付类型！！！");
	 			return;
			}
			...
			doSave1(validate,$("#myform"),url,function(val){
				...
				if(frame.length > 0){
					...
					window.location.href="${ctx}/jksq/goJksqPage";	
				}
			},function(){
				alert("抱歉，系统出错");
			});
    ...
    //计算 支出金额
    $(document).on("keyup change","[id^=txt_zcje]",function(){
    	var xmguid = $(this).parents("tr").find("#txt_jfbh").val();
    	if(xmguid==""){
    	  alert("请先选择项目");
    	  $(this).val("");
    	}else{
