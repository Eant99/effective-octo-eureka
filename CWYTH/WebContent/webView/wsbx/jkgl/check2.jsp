				...
				<div class="row">
					<div class="col-md-4">
						<div class="input-group">
							...
							<textarea style="width:100%;heigth:100%;" id="shyj" name="shyj" class="form-control input-radius"></textarea>
						</div>
					</div>
				</div>
				<div class="row">
					<div class="col-md-4">
						<div class="input-group">
							<span class="input-group-addon">是否退回申请人</span>
...
// 		var apply = $("[name='apply']").val();
		var apply=$('input:radio[name="apply"]:checked').val();
		if(shyj == ""){
			alert("请填写审核意见！");
		}else{
		$.ajax({
			type:"post",
			url:"${ctx}/jksh/doApprove",
			data:"&gid="+"${gid}"+"&operateType=TH"+"&shyj="+shyj+"&procinstid=${param.procinstid}"+"&apply="+apply,
// 			data:"&gid="+"${gid}"+"&operateType=TH"+"&shyj="+$("#shyj").val()+"&procinstid=${param.procinstid}",
			success:function(val){
				alert("操作成功！");
				getIframWindow("${param.pname}").location.href="${ctx}/jksh/goJkshPage";
				close(winId);
			}
		});
		}
