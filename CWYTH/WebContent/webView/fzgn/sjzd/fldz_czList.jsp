...
$(function (){
    ...
    table = $('#example').DataTable({
        ...
        "columns": [
            {"data": "FLH",orderable:false, 'render': function (data, type, full, meta){
            	return "<input type='checkbox' class='keyId' name='flh' value='"+data+"'><input type='hidden' class='keyId' name='flmc' value='"+full.MC+"'>";
    ...
    $(document).on("dblclick","#example tr:not(#header)",function(){
    	...
    	getIframeControl("${param.pname}","${param.czmc}").val(flmc);
    	getIframeControl("${param.pname}","${param.czmc}").focus();//手动触发验证
    	getIframeControl("${param.pname}","${param.czmc}").trigger("blur");//手动触发验证
