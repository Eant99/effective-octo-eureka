             ...
             <div class="box">
                 <div class='box-content' style="padding-bottom: 0; overflow:visible;">
                 	<div class="alert alert-info" style="padding: 6px;margin-bottom: 4px;">
			          	<strong>提示：</strong><c:if test="${param.from == 'wxjfhb'}">（1）</c:if>先找到需要的班级，然后<strong>双击</strong>这条信息;
			        </div>
...
$(function (){
	...
	var columns = [
	           	{"data": "BJBH",orderable:false, "render": function (data, type, full, meta){
	        		return '<input type="checkbox" class="keyId" name="bjbh" value="' + "("+full.BJBH+")"+full.BJMC + '">';
				},"width":10,'searchable': false},
	            {"data":"_XH",orderable:false,'render': function (data, type, full, meta){
	            	return data;
	            },"width":41,"searchable": false,"class":"text-center"},
	            {"data": "BJBH",defaultContent:""},
	            {"data": "BJMC",defaultContent:""},
	        ];
	table = $('#mydatatables').DataTable({
        ajax: {
            url: "${pageContext.request.contextPath}/xsxx/bjpage?zy=${param.zy}&nj=${param.nj}"//获取数据的方法
        },
    ...
    $(document).on("dblclick","#mydatatables tr:not(#header)",function(){
    	var val = $(this).find("[name='bjbh']").val();
    ...
    if("${param.windowModel}" == "1"){
    	...
    	$("div.button").prop("innerHTML","班级信息列表").css("font-size","14px");
    }
  	//确定按钮
   	$("#btn_sure").on("click",function(){
   		var checkbox = $("#mydatatables").find("[name='bjbh']").filter(":checked");
