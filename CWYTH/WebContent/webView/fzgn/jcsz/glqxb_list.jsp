		...
		<div role="tabpanel" class="tab-pane fade active in" id="glqxsz" aria-labelledby="home-tab">
        	<form id="glqxsz" class="form-horizontal" action="" method="post" >
				...
				<div class="row">
					<div class="col-md-12">
						<div class="title">
							<i class='fa icon-quanxianguanli' style="font-size:22px;"></i>
							<span>人员<span id="sp_ry" style="color:Red;"><c:out value="${ryxx}"/></span>的管理权限设置</span>
						</div>
					</div>
				</div>
				<div class="row">
					...
					<div class="col-md-3 dw_right">
						<c:forEach var="glqxTree" items="${glqxTree}">
				        	<div class="sc_inline number_${glqxTree.DWBH}" style="cursor:pointer;" title="双击移除：名字">
      		...
      		<form id="zclbqxsz" class="form-horizontal" action="" method="post" >
				...
				<div class="row">
					<div class="col-md-12">
						<div class="title">
							<i class='fa icon-quanxianguanli' style="font-size:22px;"></i>
							<span>人员<span id="sp_ry" style="color:Red;"><c:out value="${ryxx}"/></span>的资产类别权限设置</span>
						</div>
					</div>
				</div>
				<div class="row">
					...
					<div class="col-md-3 zc_right">
						<c:forEach var="zclbTree" items="${zclbTree}">
				        	<div class="sc_inline number_${zclbTree.BZDM}" style="cursor:pointer;" title="双击移除：名字">
								<div align="left" style="padding: 4px"><c:out value="(${zclbTree.FLH})${zclbTree.FLMC}"></c:out></div>
...
function validateBybh(val){
	...
	if(val == "D"){
		...
		if(b){
			...
			var right = '<div class="sc_inline number_'+left+'" style="cursor:pointer;" title="双击移除：名字"><div align="left" style="padding-left: 8px">'+text+'</div><input type="hidden" name="BZDM" value="'+left+'"></div>';
