...
<body>
		...
		<div class="row">
			<div class="col-md-4">
				<div class="input-group">
					...
					<input type="text" id="txt_dj" class="form-control input-radius text-right number" name="dj" value="${yshd.DJ}" onkeyup="clearNoNumB(this);">
				</div>
			</div>
			<div class="col-md-4">
				<div class="input-group">
			...
			<div class="col-md-4">
				<div class="input-group">
					<span class="input-group-addon"><span class="required">*</span>图书册数</span>
		...
		<div class="row">
			...
			<div class="col-md-4">
				<div class="input-group">
					...
					<button type="button" id="btn_cgr" class="btn btn-link btn-custom">选择
					</button></span>
					</c:if>
				</div>
			</div>
		...
		<div class="row">
			...
			<div class="col-md-4">
				<div class="input-group">
					<span class="input-group-addon" id="span_ccrq">出版日期</span>
					<input type="text" id="txt_ccrq" class="form-control date" name="ccrq" value="<fmt:formatDate value="${yshd.CCRQ}" pattern="yyyy-MM-dd"/>" data-format="yyyy-MM-dd" placeholder="出版日期">
					<span class='input-group-addon'>
						<i class="glyphicon glyphicon-calendar"></i>
					</span>
				</div>
		...
		<div class="row">
			<div class="col-md-4">
				<div class="input-group">
					...
					<c:if test="${operateType == 'U' or operateType == 'C'}">
					<span class="input-group-btn">
					<button type="button" id="btn_gbm" class="btn btn-link btn-custom">选择</button></span>
				    </c:if>
				</div>
			</div>
			<div class="col-md-4">
...
<%-- 					<c:if test="${operateType == 'U' or operateType == 'C'}"> --%>
<!-- 					<span class="input-group-btn"> -->
<!-- 					<button type="button" id="btn_jsr" class="btn btn-link btn-custom">选择</button></span> -->
<%-- 				    </c:if> --%>
				</div>
			</div>
		</div>
...
$(function(){
	...
	if(str.length>0){//必填项样式
		for(var i=0;i<str.length;i++){
			...
			}else if(modetype =="I"){//图书
				if(ctlname == "cbrq"){
					$("#txt_ccrq").parent().find("#span_ccrq").replaceWith("<span class='input-group-addon'><span class='required'>*</span>出版日期</span>");
				}else if(ctlname == "cbs"){
					$("#txt_sccj").parent().find("span").replaceWith("<span class='input-group-addon'><span class='required'>*</span>出&ensp;版&ensp;社</span>");
				}else if(ctlname == "xss"){
					$("#txt_xss").parent().find("span").replaceWith("<span class='input-group-addon'><span class='required'>*</span>销&ensp;售&ensp;商</span>");
				}else if(ctlname == "dah"){
					$("#txt_dabh").parent().find("span").replaceWith("<span class='input-group-addon'><span class='required'>*</span>档&ensp;案&ensp;号</span>");
				}else if(ctlname == "cgr"){
	...
	var validate = $('#myform').bootstrapValidator({
        fields: {
        	<%=Validate.isNullToDefault(request.getAttribute("tsjson"), "")%>
        }
    });
});
</script>
