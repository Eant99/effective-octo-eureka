		...
		<div class="row">
			...
			<div class="col-md-4">
				<div class="input-group">
					...
					<select id="drp_jzxs" class="form-control input-radius" name="jzxs" disabled>
		...
		<div class="row">
			<div class="col-md-4">
				<div class="input-group">
					...
					<input type="text" id="txt_jkdj" class="form-control input-radius text-right number" data-length="2" name="jkdj" value="<fmt:formatNumber value="${yshd.JKDJ}" pattern="0.00" />">
				</div>
			</div>
			<div class="col-md-4">
				<div class="input-group">
...
$(function(){
	...
	if(str.length>0){//必填项样式
		for(var i=0;i<str.length;i++){
			...
			}else if(modetype =="K"){//动物
				if(ctlname == "sccj"){
					$("#txt_sccj").parent().find("span").replaceWith("<span class='input-group-addon'><span class='required'>*</span>产&emsp;&emsp;地</span>");
				}else if(ctlname == "xss"){
	...
	var validate = $('#myform').bootstrapValidator({
        fields: {
        	<%=Validate.isNullToDefault(request.getAttribute("dwjson"), "")%>
        }
    });
});
//数量计算
