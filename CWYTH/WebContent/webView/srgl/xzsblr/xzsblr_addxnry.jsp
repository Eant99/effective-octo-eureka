	...
	<div class="box">
		<div class="box-panel">		
			<div class="box-header1 clearfix">
            	<div class="sub-title pull-left text-primary"><i class="fa icon-xiangxi"></i>校内人员薪资申报信息
            	</div>
				...
				<table id="mydatatables" class="table table-striped table-bordered">
		        	<thead id="thead">
				        <tr>
				            ...
				            <th style="width:300px;" id="xh"><span class="required" style="color:red">*</span>人员编号</th>
				            ...
				            <th style="width:300px;" id="zfje"><span class="required" style="color:red">*</span>银行卡号</th>
				            <th style="width:200px;" id="zfje"><span class="required" style="color:red">*</span>发放金额</th>
				            <th style="width:200px;" id="bqks">本期扣税</th>
				            <th style="width:200px;" id="sfje">实发金额</th>
				        </tr>
					</thead>
  								...
  								<c:forEach var="srmx" items="${srmx}" varStatus="sr"> 
							    	<tr id="tr_${sr.index+1}">
										...
										<span class="input-group-btn">
							    			<button type="button" id="btn_xsxx" onclick="jsxx(this)" class="btn btn-link btn-custom">选择</button>
						    			</span>
							    		...
							    		<td>
							    			<select id="txt_yhkh_${sr.index+1}" name="yhkh" class="bk input-radius">
													...
													<option value="${yhklist.khyhzh}" <c:if test="${yhklist.khyhzh == srmx.yhkh }">selected</c:if>>${yhklist.yhxx}</option>
													</c:forEach>
											</select>
							    		</td>
										<td>
										...
										<!-- 	本期扣税和实发金额 -->
										<td>
											<input type="text" id="txt_bqks_${sr.index+1}" a="a" readonly class=" input-radius sz number null txt_bqks hs" name="bqks" value="${srmx.bqks}" >
										</td>
										<td>
											<input type="text" id="txt_sfje_${sr.index+1}" a="a" readonly class=" input-radius sz number null txt_sfje hs" name="sfje" value="${srmx.sfje}" >
										</td>
							    	</tr>
				    			</c:forEach>
                          </c:when>
	                          	...
	                          	<tr id="tr_1">
							    		...
							    		<td>
							    			<select id="txt_yhkh_1" name="yhkh" class="bk input-radius">
													<c:forEach items="${yhklist}" var="yhklist">
													<option value="${yhklist.khyh}" <c:if test="${yhklist.khyhzh == dsList.dfzh }">selected</c:if>>${yhklist.klx}</option>
													</c:forEach>
											</select>
							    		</td>
										<td>
										...
										<!-- 	本期扣税和实发金额 -->
										<td>
										<input type="text" id="txt_bqks_1" a="a" readonly class=" input-radius sz  txt_bqks hs" name="bqks" value="" >
										</td>
										<td>
										<input type="text" id="txt_sfje_1" a="a" readonly class=" input-radius sz txt_sfje hs" name="sfje" value="" >
										</td>
							    	</tr>
                           </c:otherwise>
                    </c:choose>
...
$(function(){
	...
	if(operateType == 'L'||operateType == 'S'){
		...
		$("[name='fffs']").removeAttr("readonly");
		$("[name='jjkm']").removeAttr("readonly");
...
$(function(){
	...
	$(document).on("click","[class*=addBtn]",function(){
		...
		$parentTr.find("[name='bqks']").attr("id","txt_bqks_"+num);
		$parentTr.find("[name='sfje']").attr("id","txt_sfje_"+num);
...
function jsxx(bz){
	...
	//注释掉的是选择全部老师
	//select_commonWin("${ctx}/webView/fzgn/jcsz/jsxxsz/jsxx_select_list.jsp?controlId="+controlId+"&controlId2="+controlId2+"&controlId3="+controlId3+"&guid="+guid,"教师信息","900","620");
	//选择已开工资的老师
	select_commonWin("${ctx}/webView/fzgn/jcsz/jsxxsz/jsxx_xzff_list.jsp?controlId="+controlId+"&controlId2="+controlId2+"&controlId3="+controlId3+"&ffpc="+ffpc+"&guid="+guid+"&type=select","教师信息","900","620");
}
//查询银行卡
