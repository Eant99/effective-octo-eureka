    ...
    <div class="box">
		<div class='box-header'>
             <div class="row">
                 ...
                 <div class="col-md-8" >
                     <div class='alert alert-info'>
                         <strong>我的日常业务</strong>&emsp;你可以在这里找到您常用的一些功能，还可以单击<a id="btn-quick-link" class="btn btn-link" data-url="${pageContext.request.contextPath}/desk/goSetrcyw">设置我的日常业务</a>设置我的业务
                     </div>
                 </div>
             </div>
         </div>
         <div class="box-content">
             <div class='row'>
             	<c:if test="${rcyw.size()==0}"><div class='col-md-2'>暂无</div></c:if>
                         ...
                         <table class="table table-condensed table-hover">
                             ...
                             <c:forEach var="dbsx" items="${dbsx}" end="7">
                                 <tr>
                                     ...
                                     <td class="text-left"><button type='button' class="btn btn-link" data-url="${pageContext.request.contextPath}/webView/ywsh/shMain.jsp?djbh=${dbsx.DBH}&sqmkbh=${dbsx.SQMKBH}" data-mkbh="${dbsx.MKBH}" data-mkmc="待审核信息" name="dbsx"><i class="fa icon-right"></i>处理</button></td>
                               	</tr>
                             </c:forEach>
                             </tbody>
                         </table>
                             ...
                             <c:forEach var="grzc" items="${grzc}" end="4">
                                 <tr>
                                     <td style="text-align:left;">
	   									<span style="cursor:pointer" class="btn-link zclook" yqbh="${grzc.YQBH}" path="${pageContext.request.contextPath}">${grzc.YQBH}</span></td>
			...
			$.ajax({
				...
				success:function(val){
					...
					val = JSON.getJson(val);
	   					if(val.success == 'true'){
	   						parent.openRightWin(url,mkbh,title);
						}else{
							alert(val.msg);
						}
	   					close(index);
			...
			$.ajax({
				...
				success:function(val){
					...
					if(val.success=='true'){
						...
						parent.openRightWin(dom.attr("ifrurl"),mkbh,dom.attr("title"));
					}
					else{
						alert(val.msg);
					}
...
var zcxx_list = function(){
	parent.openRightWin("${pageContext.request.contextPath}/select/goMainPage","060101","我的名下资产");
...
var getYwls = function(){
	...
	$.ajax({
		type:"post",
		url:"${pageContext.request.contextPath}/desk/getYwls",
