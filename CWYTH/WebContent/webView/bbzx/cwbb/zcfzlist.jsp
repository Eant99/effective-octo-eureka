				    ...
				    <c:forEach var="list" items="${list}" varStatus="i">
				 		<tr class="tr_${i.index+1}">
				 			...
				 			<td class="num">
				 				...
				 				<input type="hidden" name="bbyf" value="${sysDate}">
		    ...
		    $(this).on("click", function() {
		    	WdatePicker({
		    		...
		    		onpicked:function(){
		    			...
		    			var jzpz = $("#jzpz").val();
		    			if(val!=""&&jzpz!=""){
		    				location.href="${ctx}/zcfz/zcfzList?status=month&bbyf="+val+"&bblx=0&jzpz="+jzpz;
		    			}
		    		},
		    		onclearing:function(){
		    			alert("请选择月份!");
...
// 	          table = getDataTableByListHj("mydatatables","${ctx}/pzlr/getPageList?treedwbh=${dwbh}",[2,'asc'],columns,0,1,setGroup);
	...
	$("#btn_print").click(function(){
		...
		var sysDate = $("[name='bbyf']").val();
		var ztgid = $("[name='ztgid']").val();
		var sfjz = $("[name='sfjz']").val();
		var jzpz = $("[name='jzpz']").val();
		location.href = "${ctx}/zcfz/goPrint?bblx="+bblx+"&bbyf="+sysDate+"&ztgid="+ztgid+"&sfjz="+sfjz+"&jzpz="+jzpz;
	});
...
//记账凭证条件切换
 $("#jzpz").change(function(){
 	...
 	var sfjz=$("#sfjz").val();
 	if(val!=""&&jzpz!=""&&sfjz!=""){
 		location.href="${ctx}/zcfz/zcfzList?bblx=0&bbyf="+val+"&bblx=1&jzpz="+jzpz+"&sfjz="+sfjz;
 	}else{
 		alert("存在空的查询条件!");
 		return;
 	}
...
// 		doExp("","${ctx}/zcfz/expExcel","资产负债表(月度)","${ctx}","");
// 	});
	//导出Excel
   	$("#btn_exp").click(function() {
   		var jzpz = $("#jzpz").val();
		...
		$.ajax({
				type : "post",
				data : {bblx:0,jzpz:jzpz,sfjz:sfjz,ztgid:ztgid,bbyf:bbyf},
				url : "${ctx}/zcfz/expExcel2",
				success : function(val) {
				 FileDownload("${ctx}/file/fileDownload","资产负债表(月度).xls",val.url);
			   }
		});
	});
