		...
		<form id="myform" class="form-inline" action="" style="padding-top: 8px;padding-bottom: 2px;">
			<div class="search-simple">
				<div class="form-group">
	                <label>选择日期</label>
	             	<input  type="text" class="input_info nd form-control" style="border:1px solid #ccc;" value="${bbnd}"/>
				    ...
				    <c:forEach var="list" items="${list}" varStatus="i">
				 		<tr class="tr_${i.index+1}">
				 			<td class="num">
				 				...
				 				<input type="hidden" name="nyr" value="${bbnd}">
				 			...
				 			<!-- 年初 -->
				 			<td class="num">
				 				<input type="text" name=qmye1 class="form-control sign-number" style="text-align: right;" value="${list.QMYE1}">
				 			</td>
				 			<!-- 年末 -->
				 			<td class="num">
				 				<input type="text" name=ncye1 class="form-control sign-number" style="text-align: right;" value="${list.NCYE1}">
				 			</td>
				 			<td class="num">
				 				${list.FZHJZC}
				 			</td>
				 			<!-- 1 -->
				 			<td class="num">
				 				<input type="text" name=qmye2 class="form-control sign-number"  style="text-align: right;"value="${list.QMYE2}">
				 				<input type="hidden" name="bblx" value="1">
				 			
				 			</td>
				 			<!-- 2 -->
				 			<td class="num">
				 				<input type="text" name=ncye2 class="form-control sign-number" style="text-align: right;" value="${list.NCYE2}">
				 			</td>
				 		</tr>
				 	</c:forEach>
				    </tbody>
		    ...
		    $(this).on("click", function() {
		    	WdatePicker({
		    		...
		    		onpicked:function(){
		    			...
		    			var sfjz = $("#sfjz").val();
		    			if(val!=""&&jzpz!=""&&sfjz!=""){
		    				location.href="${ctx}/zcfzj/zcfzbyear?bbnd="+val+"&bblx=1&jzpz="+jzpz+"&sfjz="+sfjz;
		    			}
		    		},
		    		onclearing:function(){
		    			alert("请选择月份!");
	 ...
	 //点击打印
			...
			$.ajax({
				...
				success:function(msg){
					if($.trim(msg)=="true"){
						...
						location.href = "${ctx}/zcfzj/goPrintyear?bbnd=${bbnd}&jzpz="+jzpz+"&sfjz="+sfjz+"&ztgid="+ztgid;
					}else{
						alert("预览失败!");
					}
				}
		...
		//结转凭证条件切换
		 $("#sfjz").change(function(){
		 	...
		 	var sfjz=$(this).val();
		 	if(val!=""&&jzpz!=""&&sfjz!=""){
		 		location.href="${ctx}/zcfzj/zcfzbyear?bblx=1&bbnd="+val+"&bblx=1&jzpz="+jzpz+"&sfjz="+sfjz;
		 	}else{
		 		alert("存在空的查询条件!");
		 		return;
		 	}
			...
			$.ajax({
				...
				success:function(msg){
					if($.trim(msg)=="true"){
						...
						$.ajax({
							type : "post",
							data : {bbnd:${bbnd},bblx:1,jzpz:jzpz,sfjz:sfjz},
							url : "${ctx}/zcfzj/expExcel2",
							success : function(val) {
								FileDownload("${ctx}/file/fileDownload",
										"${sysDate}年度资产负债表.xls", val.url);
							}
						});
					}else{
						alert("导出失败!");
