...
<%
String sysDate = Validate.isNullToDefaultString(request.getAttribute("sysDate"), "");
...
<div class="container-fluid" >
		<div class='responsive-table'>
			...
			<table id="mydatatables" class="table table-striped table-bordered" style="border-collapse:collapse;width:100%;margin:0 auto 100px">
		        	 <div id="print_title">
			<h2 style="text-align:center;">收入支出表（月度）</h2>
				    ...
				    <c:forEach var="list" items="${list}" varStatus="i">
				 		<tr class="tr_${i.index+1}">
				 			...
				 			<td class="num" role="bys">
				 				<input type="text" name="bys" class="form-control sign-number" style="text-align: right;" flag="${list.FLAG}" ${list.READ} value="${list.BYS}"/>
				 			</td>
				 			<td class="num" role="bnljs">
				 				<input type="text" name="bnljs" class="form-control sign-number" style="text-align: right;" flag="${list.FLAG}" ${list.READ} value="${list.BNLJS}"/>
	    ...
	    $(this).on("click", function() {
	    	WdatePicker({
	    		...
	    		onpicked:function(){
	    			...
	    			var jzpz = $("#jzpz").val();
	    			if(val!=""&&jzpz!=""){
	    				location.href="${ctx}/srfyj/toUrl?status=month&ny="+val+"&bblx=1&jzpz="+jzpz;
	    			}
	    		},
	    		onclearing:function(){
	    			alert("请选择月份!");
		...
		success:function(msg){
			if($.trim(msg)=="true"){
				...
				var bzdw = $("[name='bzdw']").val();
						$.ajax({
							type : "post",
							data : {bblx:bblx,sysDate:sysDate, sszt:sszt,jzpz:jzpz,bzdw:bzdw},
							url : "${ctx}/srfyj/expExcel2",
							success : function(val) {
								FileDownload("${ctx}/file/fileDownload","<%=sysDate %>收入支出表.xls", val.url);
							}
						});
			}else{
				alert("导出失败!");
	...
	$.ajax({
		...
		success:function(msg){
			if($.trim(msg)=="true"){
				...
				var jzpz = $("[name='jzpz']").val();
				var bzdw = $("[name='bzdw']").val();
				location.href = "${ctx}/srfyj/goPrint?status=month&bblx="+bblx+"&sysDate="+sysDate+"&sszt="+sszt+"&jzpz="+jzpz+"&bzdw="+bzdw;
...
$(".sign-number[name='bys']").on("keyup",function(){
	...
	computeLinked(14,16,13,"bys");
	computeArray(new Array(2,6,10,11,12,13),1,"bys");
	computeLinked(19,23,18,"bys");
	computeLinked(26,30,25,"bys");
	computeArray(new Array(25,31,32,33,34),24,"bys");
	computeArray(new Array(24,18),17,"bys");
	computeLinked(36,38,35,"bys");
});
$(".sign-number[name='bnljs']").on("keyup",function(){
	computeLinked(3,5,2,"bnljs");
	computeLinked(7,8,6,"bnljs");
	computeLinked(9,9,8,"bnljs");
	computeLinked(14,16,13,"bnljs");
	computeArray(new Array(2,6,10,11,12,13),1,"bnljs");
	computeLinked(19,23,18,"bnljs");
	computeLinked(26,30,25,"bnljs");
	computeArray(new Array(25,31,32,33,34),24,"bnljs");
	computeArray(new Array(24,18),17,"bnljs");
	computeLinked(36,38,35,"bnljs");
});
<!--只读-->
readonlyArray(new Array(1,16,34),"bys");
readonlyArray(new Array(1,16,34),"bnljs");
</script>
</body>
</html>