...
<!-- 				<input type="hidden" name="start" value="start" /> -->
				...
				<div class="col-md-4">
					<div class="input-group">
							...
							<input type="hidden" name="start" value="start" />
							<input type="hidden" name="zttype" id="zttype" value="3" />
							<input type="hidden" name="grwl" id="grwl" value="${dm}" />
							<input type="hidden" name="bmh" id="bmh" value="${bmh}" />
	...
	$("#btn_save").click(function(){
		...
		var json = $("#myform").serializeObject("start","end");  //json对象				
  		...
  		$.ajax({
  			...
  			type:"post",
  			complete:function(){
  				getIframWindow("${param.pname}").kz(bbqj,kmbh,kmbh,startMonth,endMonth,pz,bmh);
  				var winId = getTopFrame().layer.getFrameIndex(window.name);
  				close(winId);
  			}
  		});
