		...
		<div class='responsive-table'>
			<div class='scrollable-area' style="overflow-x:auto;"> 
				<table id="mydatatables" class="table table-striped table-bordered" style="border-collapse:collapse;width:100%;margin:0 auto" border="0">
					<div id="print_title">
					<h2 style="text-align:center;"><div style="padding-bottom:5px;margin:0 auto;display:inline-block;"><a style="color:black;text-decoration: none;">${kmmc}&nbsp;个人往来明细账</a></div></h2>
					<div id="kjkm" style="text-align:left;font-size:12px;float:left;width:31%">往来人员名称:${wlrymc }</div>
					<div id="kjkm" style="text-align:left;font-size:12px;float:left;width:30%">科目名称:<c:if test="${fn:length(kmbh)>0}">(${kmbh})</c:if>${kmmc }</div>
					<div style="text-align:center;font-size:12px;float:left;width:21%">会计期间：${bbqj}</div>
					<div style="text-align:right;font-size:12px;float:left;width:18%">单位：元</div>
					</div>
		        	<thead>
				        <tr id="header">
...
//	 	var kmbh = $("#txt_km").val();
		...
		var src = "${ctx}/grwlmxz1/getPageList?StartMonth=${StartMonth}&endMonth=${endMonth}&bbqj=${bbqj}&kmbh=${kmbh}&pageUrl=grwlmxz1&type=grwlmxz1"
			src +="&zy="+zy+"&km1="+km1+"&km2="+km2+"&PZBH1="+PZBH1+"&PZBH2="+PZBH2+"&PZRQ1="+PZRQ1+"&PZRQ2="+PZRQ2+"&dm="+dm+""
			src +="&JFJE1="+JFJE1+"&JFJE2="+JFJE2+"&DFJE1="+DFJE1+"&DFJE2="+DFJE2+"&${treesearch}&year=${year}&ryye=${ryye}";
	   var kmbh = "${kmbh}";
...
function kz(bbqj,kmbh,kmmc,StartMonth,endMonth,pz){
	...
	var DFJE1 = $("#txt_DFJE1").val();
	var DFJE2 = $("#txt_DFJE2").val();
	var src = "${ctx}/grwlmxz1/toUrl?dm="+dm+"&bbqj="+bbqj+"&kmbh="+kmbh+"&pageUrl=grwlmxz1&type=grwlmxz1";
...
function zhcx(){
	...
	var DFJE1 = $("#txt_DFJE1").val();
	var DFJE2 = $("#txt_DFJE2").val();
	var src = "${ctx}/grwlmxz1/getPageList?dm="+dm+"&bbqj=${bbqj}&kmbh=${kmbh}&pageUrl=grwlmxz1&type=grwlmxz1"
		src +="&zy="+zy+"&km1="+km1+"&km2="+km2+"&PZBH1="+PZBH1+"&PZBH2="+PZBH2+"&PZRQ1="+PZRQ1+"&PZRQ2="+PZRQ2+""
...
$("[id$=btn_sea]").click(function(){
	...
	if(dm == null||dm == ""||dm == undefined){
		...
		return;
  		...
  		select_commonWin("${ctx}/grwlmxz1/jumpWindow?bbqj=${bbqj}&kmbh=${kmbh}&StartMonth=${StartMonth}&endMonth=${endMonth}&pz=${pz}&dm=${dm}","报表条件-明细账","550","360");
	}
});      
//点击打印
...
$(document).on("click","#btn_exp",function(){
	...
	var DFJE1 = $("#txt_DFJE1").val();
	var DFJE2 = $("#txt_DFJE2").val();
	var src = "${ctx}/grwlmxz1/expExcel2?bbqj=${bbqj}&kmbh=${kmbh}&pageUrl=grwlmxz1&type=grwlmxz1"
		src +="&zy="+zy+"&km1="+km1+"&km2="+km2+"&PZBH1="+PZBH1+"&PZBH2="+PZBH2+"&PZRQ1="+PZRQ1+"&PZRQ2="+PZRQ2+""
		src +="&JFJE1="+JFJE1+"&JFJE2="+JFJE2+"&DFJE1="+DFJE1+"&DFJE2="+DFJE2+"&${treesearch}&kmbh=${kmbh}&year=${year}";
	var kmbh = "${kmbh}";
	if(kmbh==""||kmbh==null||kmbh=="undefined"){
	...
	$.ajax({
			type : "post",
			data : src,
			url : "${ctx}/grwlmxz1/expExcel2?kmbh=kmbh",
			success : function(val) {
			 FileDownload("${ctx}/file/fileDownload","${kmmc}个人往来明细账.xls",val.url);
		   }
	...
	$(document).on("dblclick","#mydatatables tr:not(#header)",function(){
		...
		if(vamc ==""){
	    	...
	    	pageSkip("${ctx}/kjhs/pzxx/pzlr","pzlrck&gs=grwlmx&ye=${ye}&guid="+vamc);
		}
	});
function sea(dm){
	select_commonWin("${ctx}/grwlmxz1/jumpWindow?bbqj=${bbqj}&kmbh=${kmbh}&StartMonth=${StartMonth}&endMonth=${endMonth}&pz=${pz}&dm="+dm,"报表条件-明细账","550","360");
