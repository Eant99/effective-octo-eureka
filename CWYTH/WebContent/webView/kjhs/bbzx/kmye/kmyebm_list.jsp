...
<!-- 					<button class="btn btn-default" type="button" id="btn1111">综合查询</button> -->
	...
	<div class="container-fluid" id="print_div" style="padding-bottom: 30px;">
		<div class='responsive-table'>
			...
			<div id="print_title">
				<h2 style="text-align:center;">部&nbsp;门&nbsp;余&nbsp;额&nbsp;表</h2>
...
function getTable(jsonStr,datestr){ 
	...
	$.ajax({
		...
		data:jsonStr+"&gs=bm&treebh=${param.dwbh}",
		dataType: "json",
		success:function(val){
			...
			for (var i=0;i<mxList.length;i++){
				 ...
				 $("tbody").append(
							"<tr>"+
							"<td name='bmmc' value='"+BMMC+"' style='text-align:left;padding:0px 1px'>"+BMMC+"</td>"+
							"<td name='kmbh' style='text-align:left;padding:0px 1px' value='"+KMBH+"'>"+KMBH+"</td>"+
							"<td name='kmmc' style='text-align:left;padding:0px 1px' value='"+KMMC+"'>"+KMMC+"</td>"+
...
//	 							"<td>"+QCYE+"</td>"+
			...
			$("tbody").append("<tr class='nojs' id='one'>"+
					"<td colspan='3' rowspan='2' style='text-align:center;padding:0px 1px'>合计</td>"+
...
// 	var bbqj = "${date}"
// // 	alert(bbqj+"ddd="+vamc);
// 	doOperate("${ctx}/mxz/toUrl?gs=bm&kmbh="+vamc+"&bbqj="+bbqj);
// });
//双击事件 id="header"
$(document).on("dblclick","#mydatatables tr:not(#header)",function(){
...
//     var endtime = year+"-"+endMonth;
	...
	doOperate("${ctx}/bmmxz1/toUrl?kmmc="+kmmc+"&kmbh="+kmbh+"&bbqj="+bbqj+"&bmh="+bmbh+"&StartMonth="+startMonth+"&endMonth="+endMonth+"&pz=1&bmye=bmye&ye=yes");
});
$("#btn_export").click(function() {
	...
	$.ajax({
		...
		url : "${ctx}/kmye/bmexpExcel",
		success : function(val) {
			var data = JSON.getJson(val);
		 FileDownload("${ctx}/file/fileDownload","部门余额表.xls",data.url);
	   }
	});
});
