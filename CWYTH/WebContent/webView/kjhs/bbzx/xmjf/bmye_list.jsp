...
<div class="fullscreen">
	...
	<div class="container-fluid" id="print_div" style="padding-bottom: 30px;">
		<div class='responsive-table'>
			...
			<div id="print_title">
				<h2 style="text-align:center;">部门余额表</h2>
...
function getTable(jsonStr,datestr){ 
	$.ajax({
		...
		data:jsonStr+"&treebh=${param.bmh}&types=bmyeb",
		dataType: "json",
		success:function(val){
			$("#span_date").html(datestr);
			$("tbody").html("");
			var mxList = val.zjList;
...
// 	var xmmc = $(this).find("[name='xmmc']").attr("value");
	...
	doOperate("${ctx}/xmjfye/toUrl?bmbh="+bmbh+"&bmmc="+bmmc+"&startMonth="+startMonth+"&endMonth="+endMonth+"&year="+year+"&jzpz="+jzpz+"&from=bmyeb&bbqj="+bbqj+"&bmh=${param.bmh}");
});
$("#btn_export").click(function() {
	...
	$.ajax({
		type : "post",
		data:$("#myform").serialize()+"&treebh=${param.dwbh}",
		url : "${ctx}/xmjfye/doExpBm",
			success : function(val) {
