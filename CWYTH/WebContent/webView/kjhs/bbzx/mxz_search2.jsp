				...
				<div class="col-md-4">
					<div class="input-group">
							...
							<select class="form-control input-radius select" name="endMonth">
								<c:forEach var="months" items="${months}">
									<option value="${months.month}" <c:if test="${ nowmonth eq months.month}">selected</c:if>>${months.month}</option>
								</c:forEach>
							</select>
							<div class="div_style" style="height:26px;line-height:26px;">
								&nbsp;&nbsp;月&nbsp;&nbsp;
					...
					<div class="col-md-4">
						<div class="input-group">
							...
							<input type="text" name="startKjkm" id="startKjkm" style="width:55% !important;" class="form-control input-radius" value=""/>
							<input type="hidden" name="Kjkmmc" id="Kjkmmc" style="width:28% ;" class="form-control input-radius" value=""/>
							<div class="div_style">
...
//   		return;
  		...
  		$.ajax({
  			url:"${ctx}/wldwmxz/paramSession",
  			data:"params="+jsonStr+"&bbqj="+bbqj,
  			dataType:"json",
  			type:"post",
  			complete:function(){
  				getIframWindow("${param.pname}").kz(bbqj,kmbh,kmbh);
  				var winId = getTopFrame().layer.getFrameIndex(window.name);
  				close(winId);
  			}
  		});
