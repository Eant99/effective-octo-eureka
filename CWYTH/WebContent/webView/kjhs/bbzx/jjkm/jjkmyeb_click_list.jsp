	...
	<div class="search" id="searchBox" style="height: 30px;">
		<form id="myform" class="form-inline" action="">
    		...
    		<div class="search-simple">
    			<div class="form-group" style="width:340px;display: none;">
					<label>会计期间</label>
				...
				<div class="form-group" style="width:250px;display: none;">
					<label>科目级次&nbsp;&nbsp;</label>
					<select class="form-control input-radius select" name="startJc">
					<option value="">请选择</option>
						<c:forEach items="${list}" var="jb1">
							<option value="${jb1.jb}" <c:if test="${startJc eq jb1.jb}">selected</c:if> >${jb1.jb}</option>
						</c:forEach>
					</select>
					至
					...
					<option value="">请选择</option>
						<c:forEach items="${list}" var="jb2">
							<option value="${jb2.jb}" <c:if test="${endJc eq jb2.jb}">selected</c:if> >${jb2.jb}</option>
						</c:forEach>
					</select>
				</div>
				...
				<button type="button" class="btn btn-primary" id="btn_chaxun" style="display: none;"><i class="fa icon-chaxun"></i>查询</button>
				
<!-- 				<div id="btn_search_more"> -->
<!-- 					<span id="btn_search_gjcx"><i class="fa icon-btn-gjcx"></i>综合查询</span> -->
<!-- 					<div class="search-more" style="width: 420px"> -->
<!-- 						<div class="form-group"> -->
<!-- 							<label>会计科目</label> -->
<!-- 							<br> -->
<%-- 							<input  type="text" name="kmbh1" types="ToBhss" id="txt_km" class="input_info  form-control" style="width:130px;" placeholder="科目编号" value="${kmbh1 }"  /> --%>
<!-- 							<button type="button" id="btn_kjkm" class="btn btn-link ">选择</button> -->
<!-- 						</div> -->
<!-- 						<div class="form-group"> -->
<!-- 							<label>起止会计科目</label> -->
<!-- 							<br> -->
<%-- 							<input   type="text" name="kmbh2" types="BHL" id="txt_km1" class="input_info  form-control" style="width:130px;" placeholder="科目编号" value="${kmbh2 }"  /> --%>
<!-- 							<button type="button" id="btn_kjkm1" class="btn btn-link ">选择</button> -->
<!-- 							<label>至</label> -->
<%-- 							<input   type="text" name="kmbh3"  types="BHH" id="txt_km2" class="input_info  form-control" placeholder="科目编号" style="width:130px;" value="${kmbh3 }"  /> --%>
...
<!-- 					<button class="btn btn-default" type="button" id="btn1111">综合查询</button> -->
	...
	<div class="container-fluid" id="print_div" style="padding-bottom: 30px;">
		<div class='responsive-table'>
			...
			<div id="print_title">
				<h2 style="text-align:center;">${param.kmmc}&nbsp;经济科目余额表</h2>
...
// 		location.href="${ctx}/jjkmyeb/jjkmList"; 
		history.back();
	});
});
...
function getTable(jsonStr,datestr){ 
	 ...
	 if(endJc<startJc){
 	 ...
 	 $.ajax({
			type:"post",
			async : false, 
			url:"${ctx}/jjkmyeb/getPageList_Click",
...
$(document).on("click","#dyyl",function(){
 ...
 function PreviewMytable(){
	...
	$(document).on("dblclick","#mydatatables tr:not(#header)",function(){
		...
		var jjbh = $(this).find("[name='kmbh']").attr("value");//经济科目编号
		var jjmc = $(this).find("[name='kmmc']").attr("value");//经济科目名称
		if(jjbh.indexOf('小计')>0)
		{
			return;
		}
	    ...
	    var bbqj = year+"年"+startMonth+"月至"+endMonth+"月";//会计区间
	    //判断是否为末级科目
	    $.ajax({
	    	type:"post",
	    	data:"jjbh="+jjbh,
	    	url:"${ctx}/jjkmyeb/checkEndJjkm",
	    	success:function(val){
	    		var data = JSON.getJson(val);
	    		if(data.success==true){
	    			doOperate("${ctx}/kjhs/jjkmmxz1/toUrl?kjkm="+kjbh+"&kmmc="+kjmc+"&bmh="+jjbh+"&bbqj="+bbqj+"&StartMonth="+startMonth+"&endMonth="+endMonth+"&pz=1&jjye=jjye&ye=yes");
	    		}else{
	    			alert("请选择末级经济科目查询明细账！");
	    		}
	    	}
	    })
	   /*  if(jjbh.length=='3' || jjbh.length=='5'){
	    	alert("请选择末级经济科目查询明细账！");
	    }else{
	    	doOperate("${ctx}/kjhs/jjkmmxz1/toUrl?kjkm="+kjbh+"&kmmc="+kjmc+"&bmh="+jjbh+"&bbqj="+bbqj+"&StartMonth="+startMonth+"&endMonth="+endMonth+"&pz=1&jjye=jjye&ye=yes");
	    } */
