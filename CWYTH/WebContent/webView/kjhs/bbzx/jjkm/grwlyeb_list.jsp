...
<!-- 					<button class="btn btn-default" type="button" id="btn1111">综合查询</button> -->
	...
	<div class="container-fluid" id="print_div" style="padding-bottom: 30px;">
		<div class='responsive-table'>
			...
			<div id="print_title">
				<h2 style="text-align:center;">个&nbsp;人&nbsp;往&nbsp;来&nbsp;余&nbsp;额&nbsp;表</h2>
...
function getTable(jsonStr,datestr){ 
	var mbmc = "${mbmc}";//模版表的模版名称，如果是1，显示模版一，如果是2，显示模版二
	 $.ajax({
			type:"post",
			async : false, 
			url:"${ctx}/grwlyeb/getPageList",
			data:jsonStr+"&treebh=${param.rybh}",
			dataType: "json",
			success:function(val){
				...
				if(mbmc=='1'){
					 $("tbody").append(
								"<tr name='guid'>"+
								"<td id='kmbh' name='kmbh' value='"+KMBH+"' rybh='"+KMBH+"' style='text-align:left;padding:0px 1px'>"+KMBH+"</td>"+
								"<td id='kmmc' name='kmmc' value='"+KMMC+"' rybh='"+KMMC+"' style='text-align:left;padding:0px 1px'>"+KMMC+"</td>"+
								"<td id='kjkmbh' name='kjkmbh'  value='"+KJKMBH+"' rybh='"+KJKMBH+"' style='text-align:left;padding:0px 1px'>"+KJKMBH+"</td>"+
								"<td id='kjkmmc' name='kjkmmc'  value='"+KJKMMC+"' rybh='"+KJKMMC+"' style='text-align:left;padding:0px 1px'>"+KJKMMC+"</td>"+
...
// 						"<td rowspan='2' colspan='2' id='qc' style='text-align:right;padding:0px 1px'>"+(data.QCYE==null||data.QCYE==0.00?"":data.QCYE)+"</td>"+
...
// 						"<td rowspan='2' colspan='2' id='qm' style='text-align:right;padding:0px 1px'>"+(data.QMYE==null||data.QMYE==0.00?"":data.QMYE)+"</td>"+
// 						"</tr>"
// 						);
				var data = val.zjList[0];
				$("tbody").append("<tr class='nojs' id='one'>"+
						...
						"<td id='qcj' style='text-align:right;padding:0px 1px'>"+(data.QCYE==null||data.QCYE==0.00?"":data.QCYE)+"</td>"+
						"<td rowspan='2' id='bqj' style='text-align:right;padding:0px 1px'>"+(data.BQJ==null||data.BQJ==0.00?"":data.BQJ)+"</td>"+
						"<td rowspan='2' id='bqd' style='text-align:right;padding:0px 1px'>"+(data.BQD==null||data.BQD==0.00?"":data.BQD)+"</td>"+
						"<td style='text-align:center;padding:0px 1px'>借</td>"+
						"<td id='qmj' style='text-align:right;padding:0px 1px'>"+(data.QMYE==null||data.QMYE==0.00?"":data.QMYE)+"</td>"+
						"</tr>"+
...
function toUrl(params,date,gs){
	if(params){
		location.href = "${ctx}/grwlyeb/jjkmList?date="+date;
	}
}
...
// 			alert("ddddddddddddddddd="+$(this).val());
			kmbh.push($(this).val());
		});
		kmbh = kmbh.join("','");
		}else{
	...
	$.ajax({
			type : "post",
			data : $("#myform").serialize()+"&treebh=${param.rybh}",
			url : "${ctx}/grwlyeb/expExcel2",
			success : function(val) {
				var data = JSON.getJson(val);
// 				console.log(val+"=====val======="+val[0]+"dffff"+data.url);
	//				alert(kmbh);
			 FileDownload("${ctx}/file/fileDownload","个人往来余额表.xls",data.url);
		   }
	}); 
});
...
//  $("#dyyl").click(function(){
 ...
 function PreviewMytable(){
	...
	$(document).on("dblclick","#mydatatables tr:not(#header)",function(){
		...
		var rybh = $(this).find("[name='kmbh']").attr("value");//经济科目编号
		var rymc = $(this).find("[name='kmmc']").attr("value");//经济科目名称;
		...
		doOperate("${ctx}/grwlmxz1/toUrl?kmbh="+kjbh+"&kmmc="+kjmc+"&rybh="+rybh+"&bbqj="+bbqj+"&StartMonth="+startMonth+"&endMonth="+endMonth+"&pz=1&ryye=ryye&ye=yes");
