		...
		<div id="btn_search_more">
					...
					<div class="search-more" style="width: 450px">
						...
						<div class="form-group hide">
							<label>会计科目</label>
							<br>
							<input  type="text" name="kjkm" types="ToBhss" id="txt_km" class="input_info  form-control" style="width:130px;" placeholder="科目编号" value=""  />
							<button type="button" id="btn_kjkm" class="btn btn-link ">选择</button>
						</div>
						<div class="form-group hide">
							<label>起止会计科目</label>
							<br>
							<input   type="text" name="kjkm" types="BHL" id="txt_km1" class="input_info  form-control" style="width:130px;" placeholder="科目编号" value=""  />
							<button type="button" id="btn_kjkm1" class="btn btn-link ">选择</button>
							<label>至</label>
							<input   type="text" name="kjkm"  types="BHH" id="txt_km2" class="input_info  form-control" placeholder="科目编号" style="width:130px;" value=""  />
...
<%-- 			<c:if test="${ymtz!='yes' }"> --%>
			<button type='button' class="btn btn-default" id="btn_back" style="display:none">返回</button>
<%-- 			</c:if> --%>
		...
		<div class="responsive-table">
			<div class="scrollable-area" style="overflow-x:auto;"> 
				<table id="mydatatables" class="table table-striped table-bordered" style="border-collapse:collapse;width:100%;margin:0 auto" border="0">
					<div id="print_title">
					<h2 style="text-align:center;"><div style="padding-bottom:5px;margin:0 auto;display:inline-block;"><a style="color:black;text-decoration: none;">${kmmc}&nbsp;经济科目明细账</a></div></h2>
		        	...
		        	<thead>
				        <tr id="header">
				            ...
				            <th style="text-align:center;width:6%" id="tr_kmbh">凭证类型</th>
				            <th style="text-align:center;width:5%" id="tr_kmbh">凭证号</th>
				            <th style="text-align:center;width:15%" id="tr_zy">摘要</th>
				            <th style="text-align:center;width:8%" id="tr_wldwmc">经济科目名称</th>
 ...
 $(function () {
	...
	//跳转时的页面控制
	 ...
	 $(document).on("click","#btn_back",function(){
		 window.history.go(-1);
// 		 doOperate("${ctx}/jjkmyeb/jjkmList?gs=&ly=jjkm&ymtz=yes"); 
	 });
	 helloWord();
	 var dm = "${dm}";
	 var zy = $("#txt_ZY").val();
...
// 		console.log('${kjkm}')
		var src = "${ctx}/kjhs/jjkmmxz1/getPageList?StartMonth=${StartMonth}&endMonth=${endMonth}&bbqj=${bbqj}&kjkm=${kjkm}&pageUrl=jjkmmxz1&type=jjkmmxz1";
			src +="&zy="+zy+"&km1="+km1+"&km2="+km2+"&PZBH1="+PZBH1+"&PZBH2="+PZBH2+"&PZRQ1="+PZRQ1+"&PZRQ2="+PZRQ2+"&dm="+dm;
			src +="&JFJE1="+JFJE1+"&JFJE2="+JFJE2+"&DFJE1="+DFJE1+"&DFJE2="+DFJE2+"&kmbh=${kmbh}&year=${year}&jjye=${jjye}&${treesearch}";
	   var kmbh = "${kjkm}";
// 	   alert(kmbh)
...
function zhcx(){
	...
	var DFJE1 = $("#txt_DFJE1").val();
	var DFJE2 = $("#txt_DFJE2").val();
	var src = "${ctx}/kjhs/jjkmmxz1/getPageList?bbqj=${bbqj}&kmbh=${kmbh}&pageUrl=jjkmmxz1&type=jjkmmxz1"
...
$("[id$=btn_sea]").click(function(){
	...
	if(dm == null||dm == ""||dm == undefined){
		...
		return;
  		...
  		select_commonWin("${ctx}/kjhs/jjkmmxz1/jumpWindow?bbqj=${bbqj}&kjkm=${kjkm}&StartMonth=${StartMonth}&endMonth=${endMonth}&pz=${pz}&dm=${dm}","报表条件-明细账","550","360");
	}
});      
//点击打印
...
//	    var bbyf = $("[name='bbyf']").val();
	...
	var DFJE1 = $("#txt_DFJE1").val();
	var DFJE2 = $("#txt_DFJE2").val();
	var src = "${ctx}/kjhs/jjkmmxz1/expExcel2?bbqj=${bbqj}&kmbh=${kmbh}&pageUrl=jjkmmxz1&type=jjkmmxz1"
		src +="&zy="+zy+"&km1="+km1+"&km2="+km2+"&PZBH1="+PZBH1+"&PZBH2="+PZBH2+"&PZRQ1="+PZRQ1+"&PZRQ2="+PZRQ2+""
		src +="&JFJE1="+JFJE1+"&JFJE2="+JFJE2+"&DFJE1="+DFJE1+"&DFJE2="+DFJE2+"&${treesearch}&kmbh=${kmbh}&year=${year}";
	/* var kmbh = "${kmbh}";
	if(kmbh==""||kmbh==null||kmbh=="undefined"){
	...
	$.ajax({
			type : "post",
			data : src,
			url : "${ctx}/kjhs/jjkmmxz1/expExcel2?kmbh=kmbh",
			success : function(val) {
			 FileDownload("${ctx}/file/fileDownload","${kmmc}经济科目明细账.xls",val.url);
		   }
	...
	$(document).on("dblclick","#mydatatables tr:not(#header)",function(){
		...
		if(vamc ==""){
    		...
    		pageSkip("${ctx}/kjhs/pzxx/pzlr","pzlrck&gs=jjkmmx&ye=${ye}&guid="+vamc);
		}
	});
function sea(dm){
	select_commonWin("${ctx}/kjhs/jjkmmxz1/jumpWindow?bbqj=${bbqj}&kjkm=${kjkm}&StartMonth=${StartMonth}&endMonth=${endMonth}&pz=${pz}&dm="+dm,"报表条件-明细账","550","360");
