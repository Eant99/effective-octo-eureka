...
<div class="container-fluid" id="print_div" >
		<div class="responsive-table">
			...
			<div id="print_title" style="width:90%;margin:10px auto 30px;">				
					<h2 style="text-align:center;">${kmmc}&nbsp;银行存款日记账</h2>
				...
				<table id="mydatatables" class="table table-striped table-bordered" style="border-collapse:collapse;width:90%;margin:10px auto 30px;" border="0" bordercolor="#FFF">
		        	<thead>
				        <tr id="header">
				            ...
				            <th  style="text-align:center;" id="tr_dfje">贷方金额</th>	
				             <th  style="text-align:center;width:5%" id="tr_fx">方向</th>
...
// 	        	   	{"data": "PZH",defaultContent:"",class:"text-left"},
// 	        	   	{"data": "ZY",defaultContent:"",class:"text-left"},
// 	        	   	{"data": "JFJE",defaultContent:"",class:"text-right"},
// 	        	   	{"data": "DFJE",defaultContent:"",class:"text-right"},
// 	        	   	{"data": "FX",defaultContent:"",class:"text-center"},
// 	        	   	{"data": "YE",defaultContent:"",class:"text-right"}
// 	        	];
// 	 debugger;
// 	  table = getDataTableByListHj("mydatatables","${ctx}/yhckrjz/getPageList?kmbh=${kmbh}&yhckrjzType=${yhckrjzType}",[],columns,0,1,"");
	
	  $.ajax({
			type:"post",
			async : false, 
			url:"${ctx}/yhckrjz/getPageList?kmbh=${kmbh}&yhckrjzType=${yhckrjzType}&year=${year}",
			dataType: "json",
				...
				for (var i=0;i<val.length;i++){
					 ...
					 $("tbody").append(
							 "<tr class=\"tr\">"+
								...
								"<td style='text-align:left;padding:0px 1px'>"+ZY+"</td>"+
								"<td style='text-align:right;padding:0px 2px'>"+JFJE+"</td>"+
								"<td style='text-align:right;padding:0px 2px'>"+DFJE+"</td>"+
								"<td style='text-align:center'>"+FX+"</td>"+
								"<td style='text-align:right;padding:0px 2px'>"+YE+"</td>"+
								"</tr>"
					);}
			},
		});	
	//综合查询
		 $("#btn_gjchaxun").click(function(){
			...
			$.ajax({
				type:"post",
				async : false, 
				url:"${ctx}/yhckrjz/getPageList?kmbh=${kmbh}&yhckrjzType=${yhckrjzType}&year=${year}&zy="+zy+"&pzbh="+pzbh+"&jfje1="+jfje1+"&jfje2="+jfje2+"&dfje1="+dfje1+"&dfje2="+dfje2,
				dataType: "json",
				success:function(val){
					$(".tr").remove();		
...
function toUrl(kmbh,type,bbqj){
	...
	location.href="${ctx}/yhckrjz/toUrl?kmbh="+kmbh+"&yhckrjzType="+type+"&pageUrl=yhckrjz&bbqj="+bbqj;
}
...
$("[id$=btn_sea]").click(function(){
 	select_commonWin("${ctx}/yhckrjz/jumpWindow","报表条件-银行存款日记账","520","400");
  });
	//打印
...
$.ajax({
		type : "post",
		data : {kmbh:kmbh},
		url : "${ctx}/yhckrjz/expExcel2?kmbh=kmbh",
		success : function(val) {
//				alert(kmbh);
		 FileDownload("${ctx}/file/fileDownload","${kmmc}银行存款日记账.xls",val.url);
	   }
});
});
//双击事件 id="header"
$(document).on("dblclick","#mydatatables tr:not(#header)",function(){
	...
	if(vamc ==""){
		...
		pageSkip("${ctx}/kjhs/pzxx/pzlr","pzlrck&gs=yhckrjz&guid="+vamc);
	}
});
