...
<div class="container-fluid" id="print_div">
   <div class="responsive-table">
			<div class="scrollable-area" style="overflow-x:auto;"> 
				...
				<div id="print_title">
					<h2 style="text-align:center;"><div style="padding-bottom:5px; margin:0 auto;display:inline-block;"><a style="color:black;text-decoration: none;">${kmmc}&nbsp;零余额日记账</a></div></h2>
...
// 	        	];
// 	 debugger;
// 	  table = getDataTableByListHj("mydatatables","${ctx}/lyerjz/getPageList?kmbh=${kmbh}&lyerjzType=${lyerjzType}",[],columns,0,1,"");
	  
	  
	  $.ajax({
			type:"post",
			async : false, 
			url:"${ctx}/lyerjz/getPageList?kmbh=${kmbh}&lyerjzType=${lyerjzType}&year=${year}",
			dataType: "json",
	...
	//综合查询
		 $("#btn_gjchaxun").click(function(){
			...
			$.ajax({
				type:"post",
				async : false, 
				url:"${ctx}/lyerjz/getPageList?kmbh=${kmbh}&lyerjzType=${lyerjzType}&year=${year}&zy="+zy+"&pzbh="+pzbh+"&jfje1="+jfje1+"&jfje2="+jfje2+"&dfje1="+dfje1+"&dfje2="+dfje2,
				dataType: "json",
				success:function(val){
					$(".tr").remove();		
...
function toUrl(kmbh,type,bbqj){
	...
	location.href="${ctx}/lyerjz/toUrl?kmbh="+kmbh+"&lyerjzType="+type+"&pageUrl=lyerjz&bbqj="+bbqj;
}
...
$("[id$=btn_sea]").click(function(){
 	select_commonWin("${ctx}/lyerjz/jumpWindow","报表条件-零余额日记账","520","400");
  });
...
// 	    var bbyf = $("[name='bbyf']").val();
 $.ajax({
 		type : "post",
 		data : {kmbh:kmbh},
 		url : "${ctx}/lyerjz/expExcel2",
 		success : function(val) {
// 				alert(kmbh);
 		 FileDownload("${ctx}/file/fileDownload","零余额日记账.xls",val.url);
 	   }
 });
 });
 ...
 $(document).on("dblclick","#mydatatables tr:not(#header)",function(){
 	...
 	if(vamc ==""){
		...
		return;
 	    ...
 	    pageSkip("${ctx}/kjhs/pzxx/pzlr","pzlrck&gs=lyerjz&guid="+vamc);
	}
 });
