		...
		<div id="btn_search_more">
					...
					<div class="search-more" style="width: 450px">
						<div class="form-group">
							<label>会计科目</label>
							<br>
							<input  type="text" name="KJKM" id="txt_KJKM" class="input_info  form-control" style="width:280px;" placeholder="会计科目" value=""  />
						</div>
						<div class="form-group">
							<label>摘要内容</label>
							<br>
						...
						<div class="search-more-bottom clearfix">
							<div class="pull-right">
								<button type="button" class="btn btn-primary" id="btn_s" onclick="zhcx()" name="btncx">
		...
		<div class='responsive-table'>
			<div class='scrollable-area' style="overflow-x:auto;"> 
				<table id="mydatatables" class="table table-striped table-bordered" style="border-collapse:collapse;width:100%;margin:0 auto" border="0">
					<div id="print_title">
					<h2 style="text-align:center;"><div style="padding-bottom:5px;margin:0 auto;display:inline-block;"><a style="color:black;text-decoration: none;">${kmmc}&nbsp;项目明细账</a></div></h2>
					<div style="text-align:left;font-size:12px;float:left;width:33%">&emsp;</div>
					<div style="text-align:center;font-size:12px;float:left;width:34%">会计期间：<span id="datedate">${bbqj}</span></div>
		        	...
		        	<thead>
				        <tr id="header">
				            ...
				            <th style="text-align:center;width:15%" id="tr_wldwmc">项目</th>
				            <th style="text-align:center;" id="tr_kjkm">会计科目</th>
				            <th style="text-align:center;" id="tr_jjfl">经济科目</th>
...
//	 	var kmbh = $("#txt_km").val();
		...
		var bmh = "${bmh}";
		console.log("----------========"+"${xmye}")
// 		var jsonStr="{\"list\":[{\"start\":\"start\",\"zttype\":\"4\",\"wlbh\":\""+dm+"\",\"year\":\""+year+"\",\"startMonth\":\""+StartMonth+"\",\"endMonth\":\""+endMonth+"\",\"startKjkm\":\"\",\"Kjkmmc\":\"\",\"jzpz\":\"1\",\"end\":\"end\"}]}"
// 	    var jsonStr="{\"list\":[{\"start\":\"start\",\"year\":\"2018\",\"startMonth\":\"01\",\"endMonth\":\"10\",\"startKjkm\":\"1\",\"Kjkmmc\":\"1\",\"jzpz\":\"1\",\"end\":\"end\"}]}"
		var jsonStr = '${params}';
	    $.ajax({
  			url:"${ctx}/xmmxz1/paramSession",
  			data:"params="+jsonStr,
  			dataType:"json",
  			type:"post",
  			complete:function(){
		...
		var src = "${ctx}/xmmxz1/getPageList?StartMonth=${StartMonth}&endMonth=${endMonth}&bbqj=${bbqj}&kmbh=${kmbh}&pageUrl=xmmxz1&type=xmmxz1";
			src +="&zy="+zy+"&km1="+km1+"&km2="+km2+"&PZBH1="+PZBH1+"&PZBH2="+PZBH2+"&PZRQ1="+PZRQ1+"&PZRQ2="+PZRQ2+"&dm="+dm;
			src +="&JFJE1="+JFJE1+"&JFJE2="+JFJE2+"&DFJE1="+DFJE1+"&DFJE2="+DFJE2+"&${treesearch}&kmbh=${kmbh}&year=${year}&xmye=${xmye}&bmh="+bmh;
	
		if(yeORno == 'yes'){
				src +="&ymtz=no&xmye=xmmx";
		}
		   ...
		   $.ajax({
				...
				success:function(val){
					for (var i=0;i<val.length;i++){
						 ...
						 var XMMC=val[i].XMMC;if(XMMC==null||XMMC =="()") XMMC='';
						 ...
						 var BMMC=val[i].BMMC;if(BMMC==null||BMMC =="()") BMMC='';
						 ...
						 $("tbody").append(
								 "<tr class=\"tr\">"+
									...
									"<td id='zy' style='text-align:left;padding:0px 1px' title='"+ZY+"'>"+ZY.substring(0,20)+"</td>"+
									"<td style='text-align:left;padding:0px 1px'>"+XMMC.substring(0,25)+"</td>"+
...
// 									"<td style='text-align:left;padding:0px 1px'>"+XMBH+"</td>"+
									...
									"<td id='fx' style='text-align:center;padding:0px 1px'>"+FX+"</td>"+
...
function kz(bbqj,kmbh,kmmc,StartMonth,endMonth,pz,bmh){
	var dm = "${dm}";
	var zy = $("#txt_ZY").val();
	var km2 = $("#txt_km2").val();
	var km1 = $("#txt_km1").val();
	...
	var DFJE1 = $("#txt_DFJE1").val();
	var DFJE2 = $("#txt_DFJE2").val();
	var src = "${ctx}/xmmxz1/toUrl?bbqj="+bbqj+"&kmbh="+kmbh+"&pageUrl=xmmxz1&type=xmmxz1";
		src +="&zy="+zy+"&km1="+km1+"&km2="+km2+"&PZBH1="+PZBH1+"&PZBH2="+PZBH2+"&PZRQ1="+PZRQ1+"&PZRQ2="+PZRQ2+"&dm="+dm+""
		src +="&JFJE1="+JFJE1+"&JFJE2="+JFJE2+"&DFJE1="+DFJE1+"&DFJE2="+DFJE2+"&StartMonth="+StartMonth+"&endMonth="+endMonth+"&pz="+pz+"&ymtz=no&bmh="+bmh;
	location.href=src;
	
}
function zhcx(){
	
	var kjkm = $("#txt_KJKM").val();
	var zy = $("#txt_ZY").val();
	var km2 = $("#txt_km2").val();
	var km1 = $("#txt_km1").val();
	var PZBH1 = $("#txt_PZBH1").val();
	...
	var DFJE1 = $("#txt_DFJE1").val();
	var DFJE2 = $("#txt_DFJE2").val();
	var src = "${ctx}/xmmxz1/getPageList?bbqj=${bbqj}&kmbh=${kmbh}&pageUrl=xmmxz1&type=xmmxz1";
		src +="&zy="+zy+"&km1="+km1+"&km2="+km2+"&PZBH1="+PZBH1+"&PZBH2="+PZBH2+"&PZRQ1="+PZRQ1+"&PZRQ2="+PZRQ2+"";
		src +="&JFJE1="+JFJE1+"&JFJE2="+JFJE2+"&DFJE1="+DFJE1+"&DFJE2="+DFJE2+"&${treesearch}&kmbh=${kmbh}&year=${year}&bmh=${bmh}&kjkm="+kjkm;
...
$("[id$=btn_sea]").click(function(){
	...
	if(dm == null||dm == ""||dm == undefined){
		...
		return;
  		...
  		select_commonWin("${ctx}/xmmxz1/jumpWindow?bbqj=${bbqj}&kmbh=${kmbh}&StartMonth=${StartMonth}&endMonth=${endMonth}&pz=${pz}&dm=${dm}","报表条件-明细账","550","360");
	}
});      
//点击打印
...
//	    var bbyf = $("[name='bbyf']").val();
	...
	var DFJE1 = $("#txt_DFJE1").val();
	var DFJE2 = $("#txt_DFJE2").val();
	var src = "${ctx}/xmmxz1/expExcel2?bbqj=${bbqj}&kmbh=${kmbh}&pageUrl=xmmxz1&type=xmmxz1"
		src +="&zy="+zy+"&km1="+km1+"&km2="+km2+"&PZBH1="+PZBH1+"&PZBH2="+PZBH2+"&PZRQ1="+PZRQ1+"&PZRQ2="+PZRQ2+""
	...
	$.ajax({
			type : "post",
			data : src,
			url : "${ctx}/xmmxz1/expExcel2?kmbh=kmbh",
			success : function(val) {
			 FileDownload("${ctx}/file/fileDownload","${kmmc}项目明细账.xls",val.url);
		   }
	...
	$(document).on("dblclick","#mydatatables tr:not(#header)",function(){
		...
		var zy =  $(this).find("[id='zy']").text();
		if(vamc ==""){
			alert("该摘要无记账凭证!");
			return;
		}else{
			pageSkip("${ctx}/kjhs/pzxx/pzlr","pzlrck&gs=xmmx&ye=${ye}&guid="+vamc);
		}
	});
function sea(dm,yeORno){
	if(yeORno == 'yes'){
// 		var zy = $("#txt_ZY").val();
// 		var km2 = $("#txt_km2").val();
// 		var km1 = $("#txt_km1").val();
...
// 		var DFJE2 = $("#txt_DFJE2").val();
// 		var pz = "1";
// 		var datedate = $("#datedate").text();
// 		var StartMonth = datedate.substring(5,7);
// 		var endMonth = datedate.substring(9,11);
// 		var src = "${ctx}/xmmxz1/toUrl?pageUrl=xmmxz1&type=xmmxz1&kmbh=1";
// 			src +="&zy="+zy+"&km1="+km1+"&km2="+km2+"&PZBH1="+PZBH1+"&PZBH2="+PZBH2+"&PZRQ1="+PZRQ1+"&PZRQ2="+PZRQ2+"&dm="+dm+""
// 			src +="&JFJE1="+JFJE1+"&JFJE2="+JFJE2+"&DFJE1="+DFJE1+"&DFJE2="+DFJE2+"&StartMonth="+StartMonth+"&endMonth="+endMonth+"&pz="+pz+"&ymtz=no&xmye=xmmx";
// 		location.href=src;
	}else{
		select_commonWin("${ctx}/xmmxz1/jumpWindow?bbqj=${bbqj}&kmbh=${kmbh}&StartMonth=${StartMonth}&endMonth=${endMonth}&pz=${pz}&bmh="+dm,"报表条件-明细账","550","240");
	}
