		...
		<form id="myform" class="form-inline" action="" style="padding-top: 8px;padding-bottom: 2px;">
			<div class="search-simple">
				<!-- <div class="form-group">
					<label>部门名称</label>
					<input type="text" id="txt_xh" class="form-control" name="bmmc" table="K" placeholder="请输入部门名称">
				</div> -->
				<!-- <div class="form-group">
					<label>经济分类</label>
					<input type="text" id="txt_xm" class="form-control" name="jjfl"  table="K" placeholder="请输入经济分类">
				</div> -->
...
//项目弹窗
    ...
    var columns = [
       ...
       {"data": "JFJE",defaultContent:"","width":120,"class":"text-right",orderable:false},
       {"data": "DFJE",defaultContent:"","width":120,"class":"text-right",orderable:false},
//        {"data": "JDFX",defaultContent:"",orderable:false},
       {"data": "JJFL",defaultContent:"","width":120,orderable:false},
     ];
    table = getDataTableByListHj("mydatatables","${ctx}/kjhs/bmmxz/getPageList?treedwbh=${dwbh}",[3,'desc'],columns,0,1,setGroup);
});
$(document).on("click","#dyyl",function(){
// $("#dyyl").click(function(){
	var kjqj1 = $("#kjqj1").val();
	var kjqj2 = $("#kjqj2").val();
	var pzzt = $("#txt_sfbhwjzpz").val();
	var data = $("#myform").serialize();
	 ...
	 $.ajax({
			...
			data:data,
			async : false, 
			url:"${ctx}/kjhs/bmmxz/getBmMxzList?treedwbh=${dwbh}&year=${year}&kjqj1="+kjqj1+"&kjqj2="+kjqj2,
			dataType: "json",
			success:function(val){
				for (var i=0;i<val.length;i++){
					 var PZRQ=val[i].PZRQ;if(PZRQ==null) PZRQ='';
					 var PZLXM=val[i].PZLXMC;if(PZLXM==null) PZLXM='';
					 var PZBH=val[i].PZBH;if(PZBH==null) PZBH='';
...
// 								"<td ><input type='checkbox' class='select-all'/></td>"+
								...
								"<td >"+BMMC+"</td>"+
								"<td id='pzrq'>"+PZRQ+"</td>"+
								"<td name='pzlx'>"+PZLXM+"</td>"+
								"<td name='pzbh'>"+PZBH+"</td>"+
								"<td id='zy'>"+ZY+"</td>"+								
...
function PreviewMytable(){
	...
	LODOP.ADD_PRINT_HTM("5%","5%","90%","85%","<h2 style='text-align:center'>部门明细账</h2>");
...
// 	$("#btn_exp").click(function(){
	...
	checkbox.each(function(){
		...
		doExp(json,"${ctx}/kjhs/bmmxz/expExcel?treedwbh=${dwbh}&guid="+guid.join("','"),"部门明细账","${pageContext.request.contextPath}",guid.join(","));
	});
