					...
					<div class="search-more" style="width: 450px">
						...
						<div class="search-more-bottom clearfix">
							<div class="pull-right">
								<button type="button" class="btn btn-primary" id="btn_se" onclick="zhcx()" name="btncx">
...
<div class="container-fluid" id="print_div" style="padding-bottom: 10px;">
		<div class='responsive-table'>
			<div class='scrollable-area' style="overflow-x:auto;"> 
				<table id="mydatatables" class="table table-striped table-bordered" style="border-collapse:collapse;width:100%;margin:10px auto 30px">
					<div id="print_title">
					<h2 style="text-align:center;"><div style="padding-bottom:5px;margin:0 auto;display:inline-block;"><a style="color:black;text-decoration: none;">${kmmc}&nbsp;部门明细账</a></div></h2>
					...
					<div style="text-align:right;font-size:12px;float:left;width:33%">单位：元&emsp;&emsp;&emsp;</div>
					</div>
		        	<thead>
				        <tr id="header">
				           <th style="text-align:center;width:7%" id="tr_pzrq">凭证日期</th>
				            <th style="text-align:center;width:3%" id="tr_kmbh">凭证类型</th>
				            <th style="text-align:center;width:5%" id="tr_kmbh">凭证号</th>
				            <th style="text-align:center;width:15%" id="tr_zy">摘要</th>
				            <th style="text-align:center;width:8%" id="tr_wldwmc">部门</th>
 ...
 $(function () {
	...
	//返回
	 $("#btn_back").click(function(){
		 window.history.go(-1);
    //doOperate("${ctx}/kmye/kmyeList?gs=bm&mkbh=070111&ymtz=yes&dwbh=${dm}"); 
	 });	
	...
	var src = "${ctx}/bmmxz1/getPageList?StartMonth=${StartMonth}&endMonth=${endMonth}&bbqj=${bbqj}&kmbh=${kmbh}&pageUrl=wldwmxz1&type=wldwmxz1"
   ...
   $.ajax({
		...
		success:function(val){
			for (var i=0;i<val.length;i++){
				 ...
				 var WLDWMC=val[i].WLDWMC;if(WLDWMC==null||WLDWMC=="()") WLDWMC='';
				 var KMBH=val[i].KMBH;if(KMBH==null) KMBH='';
				 var KMMC=val[i].KMMC;if(KMMC==null||KMMC=="()") KMMC='';
				 var JJFLMC=val[i].JJFLMC;if(JJFLMC==null||JJFLMC=="()") JJFLMC='';
				 ...
				 $("tbody").append(
						 "<tr class=\"tr\">"+
							...
							"<td style='text-align:left;padding:0px 1px'>"+JJFLMC+"</td>"+
...
function kz(bbqj,kmbh,kmmc,StartMonth,endMonth,pz){
	...
	var DFJE1 = $("#txt_DFJE1").val();
	var DFJE2 = $("#txt_DFJE2").val();
	var src = "${ctx}/bmmxz1/toUrl?bbqj="+bbqj+"&kmbh="+kmbh+"&pageUrl=wldwmxz1&type=wldwmxz1";
		src +="&zy="+zy+"&km1="+km1+"&km2="+km2+"&PZBH1="+PZBH1+"&PZBH2="+PZBH2+"&PZRQ1="+PZRQ1+"&PZRQ2="+PZRQ2+"&dm="+dm+""
...
function zhcx(){
	...
	var DFJE1 = $("#txt_DFJE1").val();
	var DFJE2 = $("#txt_DFJE2").val();
	var src = "${ctx}/bmmxz1/getPageList?bbqj=${bbqj}&kmbh=${kmbh}&pageUrl=wldwmxz1&type=wldwmxz1"
		src +="&km="+km+"&zy="+zy+"&km1="+km1+"&km2="+km2+"&PZBH1="+PZBH1+"&PZBH2="+PZBH2+"&PZRQ1="+PZRQ1+"&PZRQ2="+PZRQ2+""
...
function sea(dm){	
	...
	var year = datedate.substring(0,4);
	var StartMonth = datedate.substring(5,7);
	var endMonth = datedate.substring(14,16);
	...
	$.ajax({
			...
			type:"post",
			complete:function(){
				var src = "${ctx}/bmmxz1/toUrl?pageUrl=wldwmxz1&type=wldwmxz1&dm="+dm+"&StartMonth="+StartMonth+"&endMonth="+endMonth+"&ymtz=no";
				location.href=src;
			}
		});
}
$("[id$=btn_sea]").click(function(){
	...
	if(dm == null||dm == ""||dm == undefined){
		...
		return;
  		...
  		select_commonWin("${ctx}/bmmxz1/jumpWindow?bbqj=${bbqj}&kmbh=${kmbh}&StartMonth=${StartMonth}&endMonth=${endMonth}&pz=${pz}&dm=${dm}","报表条件-明细账","550","360");
	}
});      
//点击打印
...
$(document).on("click","#btn_exp",function(){
	...
	var DFJE1 = $("#txt_DFJE1").val();
	var DFJE2 = $("#txt_DFJE2").val();
	var src = "${ctx}/bmmxz1/expExcel2?bbqj=${bbqj}&kmbh=${kmbh}&pageUrl=wldwmxz1&type=wldwmxz1";
	...
	$.ajax({
			type : "post",
			data : src,
			url : "${ctx}/wldwmxz1/expExcel2?kmbh=kmbh&foo='部门明细账'",
			success : function(val) {
			 FileDownload("${ctx}/file/fileDownload","${kmmc}部门明细账.xls",val.url);
		   }
...
$(document).on("dblclick","#mydatatables tr:not(#header)",function(){
	...
	if(vamc ==""){
		...
		pageSkip("${ctx}/kjhs/pzxx/pzlr","pzlrck&gs=bmmx&ye=${ye}&guid="+vamc);
	}
});
