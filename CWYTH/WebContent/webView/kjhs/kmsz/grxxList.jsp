             ...
             <div class="box">
                 <div class='box-content' style="padding-bottom: 0; overflow:visible;">
                 	<div class="alert alert-info" style="padding: 6px;margin-bottom: 4px;">
			          	<strong>提示：</strong><c:if test="${param.from == 'wxjfhb'}">（1）</c:if>先找到需要的人员，然后<strong>双击</strong>这条信息;
...
$(function (){
	...
	var columns = [
	              ...
	              {"data": "XM",'render':function(data,type,full,meta){
	           		return '<a  name="xm" rybh="'+full.XM+'" path="${ctx}">'+ data +'</a>';
	              },"width":50},
...
// 	               },orderable:false,"width":220,class:"text-center"}
	...
	table = $('#mydatatables').DataTable({
        ajax: {//${ctx}/ryb/getPageList?treedwbh=${dwbh}  ${pageContext.request.contextPath}/bmysbz/dwxx?
            url: "${pageContext.request.contextPath}/ryb/getPageList1?treedwbh=${dwbh}&dwbh=${param.dwbh}&a=${param.a}&treesearch=${param.treesearch}&from=${param.from}&dwxx=${param.dwbh}"//获取数据的方法
        },
    ...
    $(document).on("dblclick","#mydatatables tr:not(#header)",function(){
    	var val = $(this).find("[name='id']").attr("rygh");
    	var guid = $(this).find("[name='xm']").attr("rybh");
    	
      	if(val==''||val==null||val=='undefined'){
    		alert("没有可以选择的数据！");
    	}else{
    		getIframeControl("${param.pname}","${param.controlId1}").val(val);
    		getIframeControl("${param.pname}","${param.controlId}").val(guid);
