...
$(function(){
	...
	$(document).on("click","#btn_addtj",function(){
		...
		var dm2 = $("#mydatatables").find("[name='guid']").filter(":checked").attr("kmbh");
   		if(checkbox.length>1){
   			...
   			dm=dm2;
   	   		doOperate("${ctx}/kmsz/goEditJjPage?type=tj&kmjc="+kmjcr+"&dm="+dm+"&l="+l+"&k="+k,"C");   			
   		}else{
   			if(dm==""){
   	 			alert("请选择左侧科目或者右侧列表节点！");
   	 			return;
   	 		}	
   	   		doOperate("${ctx}/kmsz/goEditJjPage?type=tj&kmjc="+kmjc+"&dm="+dm+"&l="+l+"&k="+k,"C");
   		}
	...
	//修改
 	 $(document).on("click",".btn_upt",function(){
 		var guid = $(this).attr("guid");
  		doOperate("${ctx}/kmsz/goEditJjPage?guid="+guid,"U");
	});
	
 	//查看
 	 $(document).on("click",".btn_look",function(){
 		var guid = $(this).attr("guid");
  		doOperate("${ctx}/kmsz/goLookJjPage?guid="+guid,"L");
	});
 	$(document).on("click","#btn_addxj",function(){
		...
		if(checkbox.length>1){
    		...
    		dm=dm2;
    		doOperate("${ctx}/kmsz/goEditJjPage?type=xj&kmjc="+(parseInt(kmjcr)+1)+"&dm="+dm+"&l="+l+"&k="+k,"C");
   		}else{
   			if(dm==""){
   	 			alert("请选择左侧科目或者右侧列表节点！");
   	 			return;
   	 		}	
   	   		doOperate("${ctx}/kmsz/goEditJjPage?type=xj&kmjc="+kmjc+"&dm="+dm+"&l="+l+"&k="+k,"C");
   		}
   	});
	//单条删除
		...
		$.ajax({
			...
			success:function(val) {
			  if(val.success == "true"){
				 ...
				 doDel("guid="+guid+"&kmbh="+kmbh,"${ctx}/kmsz/doDelete",function(val){
	...
	var columns = [
		...
		{"data": "QYF",defaultContent:"","class":"text-center"},
   ...
   table = getDataTableByListHj("mydatatables","${ctx}/kmsz/getJjkmPageList?treeDm=${param.kmbh}&kmnd=${kmnd}&treesearch=${treesearch}",[2,'asc'],columns,0,1,setGroup);
    ...
    var tableRelod = function(table) {
        ...
        table.ajax.url("${ctx}/kmsz/getJjkmPageList?kmnd="+kmnd);
        table.ajax.reload();
    };
