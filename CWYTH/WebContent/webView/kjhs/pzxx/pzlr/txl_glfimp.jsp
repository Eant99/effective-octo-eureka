...
<body>
<form id="myform"  class="form-horizontal" action="${ctx}/kjhs/pzxx/pzlr/uploadGlf?pname=${param.pname}&zbid=${param.zbid}&mxid=${param.mxid}" method="post" enctype="multipart/form-data">
	<input type="hidden" id="sclx" name="sclx" value="">
    <div class="page-header">
		<div class="pull-left">
			<span class="page-title text-primary"><i class='fa icon-xiangxixinxi'></i>明细信息导入</span>
		 </div>
	</div>
...
$(function() {
	if("${bool}"=="true"){
		...
		getIframWindow("${param.pname}").reload();
    	close(winId);
	}else if("${bool}"=="false"){
		alert("${err}");
		var pname = "${pname}";
		var winId = getTopFrame().layer.getFrameIndex(window.name);
		getIframWindow("${pname}").reload();
    	close(winId);
	}
	//下载模板
	$("#btn_exp").click(function(){
		var fname = "科研管理费信息.xls";
		var path = "uploadFiles/科研管理费信息.xls";
