	...
	function PreviewMytable(){
		var guid = [];
			...
			$('div[id^=divs]').each(function(i){
				...
				for(var kk=0;kk<trCounts;kk++){
					trHeight += $(document.getElementById("divs"+i)).find('table tr').eq(kk).outerHeight();
				...
				if(trHeight>500){
					for(var jj=0;jj<index.length;jj++){
						...
						$(document.getElementById("divs"+i)).find('table tr').each(function(j){
							var $this = $(this);
							if(jj!=index.length-1){
								if(($this.index()>=(index[jj]))&&($this.index()<(index[jj+1]))&&$this.index()!=0){
									newDoms.find('table').append($this.clone());
									console.log(jj);
								}
							}else{
								if(($this.index()>=(index[jj]))){
									newDoms.find('table').append($this.clone());
									console.log(jj);
								}
							}
						...
						newDoms.find('span[id=indexNums]').text((jj+1));
						newDoms.find('span[id=sumNums]').text('/'+index.length);
