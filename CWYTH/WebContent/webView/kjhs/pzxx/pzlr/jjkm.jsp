	...
	<div class="search" id="searchBox">
		<form id="myform" class="form-inline" action="" style="padding-top:8px" onkeydown="if(event.keyCode==13)return false;">
    		<div class="search-simple">
				...
				<div class="form-group">
					<label>科目名称</label>
					<input type="text" id="txt_xm" class="form-control input-radius" name="kmmc"  table="j" placeholder="请输入科目名称" >
				</div>
...
var columns = [
  ...
  table = getDataTableByListHj("mydatatables",target+"/getJjkmPageList?treeDm=${param.kmbh}&isAll=${param.isAll}&xmguid=${param.xmguid}&kmnd=${kmnd}&treesearch=${treesearch}",[2,'asc'],columns,0,1,setGroup);
$(function(){
	var pname = "${param.pname}";
	if(pname=="iframe_110201"||pname=="iframe_112001"||pname=="iframe_110203"||pname=="iframe_980130"){
		...
		$(document).on("dblclick","tr:not(#header)",function(){
			  ...
			  $.ajax({
				  url:"${ctx}/kjhs/pzxx/pzlr/doSelectSfmj?kmbh="+jjfl,
					  ...
					  success:function(val){
					        	if(val=="0"){
					        		getIframeControlForPzlr("${param.pname}","${param.controlId}").val(jjfl);
					        		getIframeControlForPzlr("${param.pname}","${param.controlId}").attr("title",jjflmc);
