...
<body>
	...
	<div class='page-header'>
        ...
        <div class="pull-right">
        	<button type="button" class="btn btn-default" id="btn_th"><i class="fa icon-saves"></i>退回</button>
			 <button type="button" class="btn btn-default" id="btn_fh"><i class="fa icon-saves"></i>复核</button>
			 <button type="button" class="btn btn-default" id="btn_plfh"><i class="fa icon-saves"></i>批量复核</button>
...
$(document).ready(function(){
	...
	$(".zckm").hide();
	$(".srkm").hide();
	var jsfs = $(".pzfz_info").find(".jsfs").val();
	if(jsfs == "0005"){
		$(".xingming").show();
		$(".beizhu").hide();
	}else{
...
$("#btn_fh").click(function(){
	if(userId == zdrId){
		alert("制单人及复核人员不能为同一个人，请确认!");
		return;
	}
	var kmbh = $("#mxForm").find("[name='kmbh']");
	$.each(kmbh,function(){
		if($(this).val()==""){
			alert("请补充科目编号为空的信息！");
			return;
...
$("#btn_plfh").click(function(){
	var guid = $("#guid").val();
	var pzz=$("#txt_pzz").val();
	select_commonWin(target + "/plfhwindow?guid=" + guid+"&pzz="+pzz,"凭证复核", "600", "550");
});
$("#btn_th").click(function(){
			var guid = $("#guid").val();
			var pzz=$("#txt_pzz").val();
			var pzbh=$("#txt_pzbh").val();
			select_commonWin(target + "/goplthwindow?guid=" + guid+"&jsr="+zdrId,"退回原因", "600", "250");
		});
$("#btn_qxfh").click(function(){
	var guid = $("#guid").val();
	var pzzt = $("[name='pzzt']").val();
	select_commonWin(target + "/fhyj?guid=" + guid+ "&pzzt=" + pzzt, "复核意见", "600", "350");
});
function toUrl(){
