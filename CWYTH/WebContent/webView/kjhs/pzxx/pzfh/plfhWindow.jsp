		...
		<div class="box-panel">		
			...
			<div class="container-fluid box-content">
				<div class="row">
					<div class="col-md-1">
						<div class="input-group">
							<span class="input-group-addon" style="border: 1px solid #ccc!important;vertical-align: middle; "><span class="required">*</span>起始日期</span>
							...
							<span class="input-group-addon" style="border: 1px solid #ccc!important;vertical-align: middle; "><span class="required">*</span>结束日期</span>
...
// 		})
		...
		$("#btn_save").click(function(){
			...
			var p1 = $("[name='jspdh']").attr("disabled");
			var p2 = $("[name='jsrq']").attr("disabled");
			if(p1 == undefined  && p2 == undefined){
				alert("按编号和按日期请至少选择一个!");
				return;
			}
			if(qspdh !== "" && jspdh !== "" && qsrq == "" && jsrq == ""){
				if(zdr == ""){
					$.ajax({
						type:"post",
						url:target+"/plfh?type=zdrwk",
						//dataType:"json",
						data:"guid="+guid+"&qspdh="+qspdh+"&jspdh="+jspdh,
						success:function(data){
							if(data){
								alert("按照凭证编号批量复核成功,制单人及复核人员为同一个人的单据没有进行复核！");
					...
					$.ajax({
						type:"post",
						url:target+"/plfh?type=zdr",
						//dataType:"json",
						data:"guid="+guid+"&qspdh="+qspdh+"&jspdh="+jspdh+"&zdr="+zdr,
						success:function(data){
							if(data){
								alert("按照凭证编号批量复核成功！");
								getIframWindow("${param.pname}").toUrl();
				...
				if(zdr == ""){
					$.ajax({
						type:"post",
						url:target+"/plfhrq?type=zdrwk",
						//dataType:"json",
						data:"guid="+guid+"&qsrq="+qsrq+"&jsrq="+jsrq,
						success:function(data){
							if(data){
								alert("按照凭证日期批量复核成功！");
								getIframWindow("${param.pname}").toUrl();
					...
					$.ajax({
						type:"post",
						url:target+"/plfhrq?type=zdr",
						//dataType:"json",
						data:"guid="+guid+"&qsrq="+qsrq+"&jsrq="+jsrq+"&zdr="+zdr,
						success:function(data){
							if(data){
								alert("按照凭证日期批量复核成功！");
				  				close(winId);
