...
//  this.defaultTimeZone = (new Date()).toString().split('(')[1].slice(0,-1);//在ie下得出的时间不带后面的时区名称
...
//    	$("#" + this.element[0].id).change();
    ...
    place: function () {
      ...
      top = top;// - containerOffset.top//这部分其实是控件的高度，减去这部分之后，谷歌浏览器上日期控件的弹窗会定位在文本框的上边
      left = left;// - containerOffset.left//这部分其实是控件的宽度，减去这部分之后，谷歌浏览器上日期控件的弹窗会定位在文本框的上边
  ...
  var DPGlobal = {
    ...
    getDefaultFormat: function (type, field) {
      if (type == 'standard') {
        if (field == 'input')
          return 'yyyy-mm-dd';//默认改成只有日期，没有时间的类型（原版：yyyy-mm-dd hh:ii）
        else
          return 'yyyy-mm-dd';//原版：yyyy-mm-dd hh:ii:ss
      } else if (type == 'php') {
        if (field == 'input')
          return 'Y-m-d H:i';
        else
