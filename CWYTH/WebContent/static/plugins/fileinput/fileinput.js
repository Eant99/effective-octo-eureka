...
(function (factory) {
    ...
    defaultFileActionSettings = {
        showRemove: true,
        showUpload: true,
        showZoom: false,//预览改成新页面展示了，所以这里不要预览按钮
        showDrag: false,//已上传的图片左下角的图标，原本是移动/重置，因为我们的图片都不能移动，所以不显示这个图标了
    ...
    tPreview = '<div class="file-preview {class}">\n' +
        ...
        '    </div>\n' +
        '</div>';
    tClose = '<div class="close fileinput-remove" title="点击删除所有未上传的图片">&times;</div>\n';
    ...
    //注意：{caption}后边的{size}很重要，去掉会获取不到djlx
    tFooter = '<div class="file-thumbnail-footer">\n' +
    '<div class="file-footer-caption" title="{caption}">{caption} {size}</div>\n' +
    '    {progress} {actions}\n' +
    '</div>';
    //由于即使带着这个文本框，也不能修改文件名称，所以干脆去掉了，上传之后，文件按照图片的名称命名
//    tFooter = '<div class="file-thumbnail-footer">\n' +
...
//        '</div>';
    ...
    tImage = '<img src="{data}" class="kv-preview-data file-preview-image" title="点击预览图片 {caption}" alt="{caption}" ' +
        STYLE_SETTING + '>\n';
...
//              out = (size / Math.pow(1024, i)).toFixed(2) * 1 ;//+ ' ' + sizes[i]
    ...
    $(document).ready(function () {
        ...
        $(document).on("mouseover","img.file-preview-image",function(){
        	if($(this).attr("src").substr(0,4) == "data"){
        		var title = $(this).attr("title");
        		$(this).attr("title",title.replace("点击预览图片 ",""));
        	}
        	else{
        ...
        $(document).on("click","img.file-preview-image",function(){
        	var dqurl = encodeURIComponent($(this).attr("src"));
        	var list = [];
        	var cnt = 0;
        	$("img.file-preview-image").each(function(){
        		...
        		if(imgurl.substr(0,4) == "data"){
        			...
        			list.push("{name:'" + $(this).attr("alt") + "',url:'" + imgurl + "'}");
        		}
        	});
        	/*if(cnt > 0){
        		alert(cnt + "张图片没有上传，请先保存！");
        	}*/
        	if(cnt > 0){
        		alert("请先保存图片信息！");
        	}
...
//	        	var imglist = encodeURIComponent("[" + list.join(",") + "]");
//	        	var url = "http://" + window.location.host + getTopFrame().window.sessionXMM + "/webView/imgShow.jsp?dqurl="+dqurl+"&imglist="+imglist;
//	        	window.open(url,"图片信息");
        	}
        });
    });
}));