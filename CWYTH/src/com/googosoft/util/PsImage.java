...
public class PsImage {
    ...
    public PsImage clip(int srcX,int srcY,int width,int height){
        ...
        g.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
        //左上角：x：srcX，y:srcY
        //右上角：x：srcX + width，y:srcY
        //左下角：x：srcX，y:srcY + height
        //右下角：x:srcX + width，y:srcY + height
    ...
    /**
     * 图片旋转（目前只能顺时针旋转，逆时针旋转报错）
     * angle：角度，图片旋转角度
     * @param angle
     * @return
     */
    public PsImage rotate(float angle){
        int width = this.dealedImage.getWidth();
        int height = this.dealedImage.getHeight();
        int new_w = 0, new_h = 0;
        int new_radian = (int)angle;
        if(angle < 0){
        	new_radian = 360 + (int)angle;
        }
        if (new_radian <= 90) {
        ...
        } else if (new_radian <= 180) {
            new_radian = new_radian - 90;
        ...
        } else if (new_radian <= 270) {
            new_radian = new_radian - 180;
            new_w = (int) (width * Math.cos(Math.toRadians(new_radian)) + height * Math.sin(Math.toRadians(new_radian)));
            new_h = (int) (height * Math.cos(Math.toRadians(new_radian)) + width * Math.sin(Math.toRadians(new_radian)));
        } else {
            new_radian = new_radian - 270;
            new_w = (int) (height * Math.cos(Math.toRadians(new_radian)) + width * Math.sin(Math.toRadians(new_radian)));
            new_h = (int) (width * Math.cos(Math.toRadians(new_radian)) + height * Math.sin(Math.toRadians(new_radian)));
        }
    ...
    private AffineTransform findTranslation(AffineTransform at, BufferedImage bi, float angle) {//45
