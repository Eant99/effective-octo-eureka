...
@Repository("jjkmmxz1Dao")
public class Jjkmmxz1Dao extends BaseDao {
	/**
	 * 查询月份
	 * 
	 * @return
	...
	public String getSql(String kmbh,String kjzd,String treesearch,PageData pd){
		...
		sql.append(" FROM CWPT_MXZ T left join gx_sys_dwb d on d.dwbh=t.bmbh left join cw_xmjbxxb x on t.xmbh=x.xmbh and x.bmbh=t.bmbh WHERE T.LOGIN='"+LUser.getRybh()+"' ");
...
//		pd.getString(key)
		...
		sql.append(")t");
		sql.append(" GROUP BY T.GUID,t.wldwmc,T.PZBHGUID,T.KMBH,T.KMMC, T.PZH,t.pzlxmc, T.ZY, T.JJFL, T.BMBH,T.XMBH,PZRQ,JFJE,DFJE,QCYE,YE,FX,xh,BMMC,XMMC ");
...
//			}
	...
	public boolean getResultNew(String jjbh, String sszt, String kjkm,
		...
		boolean bool = false;
			...
			if(Validate.noNull(PZBH1)){
				sql.append(" and t.PZBH >= '"+PZBH1+"'");
			}
			if(Validate.noNull(PZBH2)){
				sql.append(" and t.PZBH <= '"+PZBH2+"'");
			}
			if(Validate.noNull(PZRQ1)){
				sql.append(" and t.pzrq >= to_date('"+PZRQ1+"','yyyy-MM-dd')");
			}
			if(Validate.noNull(PZRQ2)){
				sql.append(" and t.pzrq<= to_date('"+PZRQ1+"','yyyy-MM-dd')");
			}
			if(Validate.noNull(JFJE1)){
				sql.append(" and t.JFJE >= '"+JFJE1+"'");
			}
			if(Validate.noNull(JFJE2)){
				sql.append(" and t.JFJE <= '"+JFJE2+"'");
			}
			if(Validate.noNull(DFJE1)){
				sql.append(" and t.DFJE >= '"+DFJE1+"'");
			}
			if(Validate.noNull(DFJE2)){
				sql.append(" and t.DFJE <= '"+DFJE2+"'");
			}
			parInList.add(sql.toString());
