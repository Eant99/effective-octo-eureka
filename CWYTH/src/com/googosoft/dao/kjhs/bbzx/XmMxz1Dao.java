...
@Repository("xmmxz1Dao")
public class XmMxz1Dao extends BaseDao {
	/**
	 * 查询月份
	 * 
	 * @return
	...
	public String getSql(String kmbh,String kjzd,String treesearch,PageData pd){				
		...
		sql.append(" (select  '('||xmbh||')'||xmmc  from cw_xmjbxxb where guid = t.xmguid) xmmc,");
		sql.append(" T.ZY,");
		sql.append(" T.KMBH,");
		sql.append(" T.JJFL,");
		sql.append(" T.BMBH,");
...
//		sql.append(" GROUP BY T.GUID,t.wldwmc,T.PZBHGUID,T.KMBH,T.KMMC, T.PZH,t.pzlxmc, T.ZY, T.JJFL, T.BMBH,T.XMBH,PZRQ,JFJE,DFJE,QCYE,YE,FX,xh");
		if(Validate.noNull(pd.getString("bmh"))){
			sql.append("   ORDER BY kmbh,pzrq,pzh,xh ");
		}else{
	 ...
	 * @return
	 */
	public String getSql(String kmbh,String kjzd,String treesearch,PageData pd,String sszt){				
		StringBuffer sql = new StringBuffer();
		sql.append(" select * from(");
		sql.append(" SELECT");
		sql.append(" T.GUID,T.PZBHGUID,");
		...
		sql.append(" T.KMBH,");
		sql.append(" T.JJFL,");
		sql.append(" (select  '('||kmbh||')'||kmmc  from CW_JJKMB where kmbh = t.jjfl and sszt ='"+sszt+"') jjkmmc,");
		sql.append(" T.BMBH,");
...
//		sql.append(" GROUP BY T.GUID,t.wldwmc,T.PZBHGUID,T.KMBH,T.KMMC, T.PZH,t.pzlxmc, T.ZY, T.JJFL, T.BMBH,T.XMBH,PZRQ,JFJE,DFJE,QCYE,YE,FX,xh");
	...
	public List<Map<String, Object>> getPageList(String kmbh,String kjzd,String treesearch,PageData pd,String sszt) {
		String sql=getSql(kmbh,kjzd,treesearch,pd,sszt);
		return db.queryForList(sql);
	}
	/**
	 * 查询级别list
	...
	public boolean getResult(List list,String sszt,String kjzd,String kmbh,PageData pd) {
		...
		for(int i=0;i<list.size();i++){
			...
			if(Validate.isNull(wlbh)){
				wlbh = Validate.isNullToDefaultString(map.get("grwl"), "");
			}
			if(Validate.noNull(pd.getString("bmh"))){
				wlbh = Validate.isNullToDefaultString(pd.getString("bmh"), "");
			}
			if("1".equals(kmbh)){
				kmbh = "";
			}
	 ...
	 * @return
	 */
	public boolean getResultNew(String xmbh ,String sszt,String nian,String StartMonth,String EndMonth,PageData pd) {
		String login = LUser.getGuid();
		String procName = "PRO_CWYTH_MXZ_WLDWNEW";
		List parInList = new ArrayList<String>();
		boolean bool = false;
			...
			String type = "3";
			//往来单位编号
			String wlbh = Validate.isNullToDefaultString(xmbh, "");
	 ...
	 * @return
	 */
	public boolean getResultnew(String xmbh ,String sszt,String kmbh,String nian,String StartMonth,String EndMonth,PageData pd) {
		String login = LUser.getGuid();
		String procName = "PRO_CWYTH_MXZ_WLDWNEW";
		List parInList = new ArrayList<String>();
		boolean bool = false;
