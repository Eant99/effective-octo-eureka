...
@Repository("mxzDao")
public class MxzDao extends BaseDao {
	/**
	 * 查询月份
	 * 
	 * @return
...
//		}
	...
	public boolean getResult(List list,String sszt,String kjzd,String kmbh, PageData pd) {
		...
		for(int i=0;i<list.size();i++){
			...
			if(Validate.noNull(PZBH1)){
				sql.append(" and to_number(pz.PZBH) >= '"+PZBH1+"'");
			}
			if(Validate.noNull(PZBH2)){
				sql.append(" and to_number(pz.PZBH) <= '"+PZBH2+"'");
			}
			if(Validate.noNull(PZRQ1)){
	...
	public List GetModels(String dwbh) {
		String sql = "select t.*,(select dw.mc from gx_sys_dwb dw where dw.dwbh=t.bmbh) from cw_xmjbxxb t where t.bmbh='"+dwbh+"' order by xmbh";
		return db.queryForList(sql);
	}
	/**
	 * 判断存储过程时候执行完毕
	 ...
	 * @return
	 */
	public boolean getResultNew(String sszt, String kmbh, String nian,
