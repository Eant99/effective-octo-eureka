...
@Repository
public class PzfhDao extends BaseDao{
	...
	public void updateXmMoneyByPz(String guid){
		...
		for(int i=0,len=list.size();i<len;i++){
			...
			String pzbh = Validate.isNullToDefaultString(map.get("PZBH"), "");
			if(Validate.noNull(xmbh)&&Validate.noNull(bmbh)){
				//若果是6类 预算类  预算金额 和 剩余金额 都加钱
				if("6".equals(kmbh.substring(0, 1))){
					StringBuffer sqlAll = new StringBuffer();
					sqlAll.append(" update cw_xmjbxxb t set");
					sqlAll.append(" t.ysje=");
					sqlAll.append(" ysje + (nvl('"+dfje+"',0)-nvl('"+jfje+"',0))");
					sqlAll.append(" where t.xmbh='"+xmbh+"' and t.bmbh='"+bmbh+"'");
					sqlList.add(sqlAll.toString());
				}
				...
				sqls.append(" syje + (nvl('"+dfje+"',0)-nvl('"+jfje+"',0))");
			...
			sqlBuffer.append(" (?,'',?,?,(select t.bmmc from cw_pzzsb t where t.guid='"+mxguid+"'),?,(select t.xmmc from cw_pzzsb t where t.guid='"+mxguid+"'),?,SysDate,?,(select t.kjqj from CW_PZLRZB t where t.guid='"+pzbh+"'),?,?,to_number(?),to_number(?),?,(select t.xmye from cw_pzzsb t where t.guid='"+mxguid+"')) ");
			Object[] obj = {
	...
	//获取期末结账会计期间
		...
		public List<Map<String, Object>> getPzlrMain(String qspdh,String jspdh, String qsrq, String jsrq, String zdr,String pzzt, String sszt, String kjzd) {
			...
			if(Validate.noNull(qspdh)&&Validate.noNull(jspdh)){
				sql += " and (pzbh between '"+qspdh+"' and '"+jspdh+"') ";
			}
			if(Validate.noNull(qsrq)&&Validate.noNull(jsrq)){
				sql += " and pzrq between to_date('"+qsrq+"','yyyy/mm/dd') and to_date('"+jsrq+"','yyyy/mm/dd') ";
			}
