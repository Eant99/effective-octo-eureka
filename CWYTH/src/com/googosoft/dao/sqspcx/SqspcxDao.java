package com.googosoft.dao.sqspcx;
...
@Repository("sqspcxDao")
public class SqspcxDao extends BaseDao{
	private Logger logger = Logger.getLogger(SqspcxDao.class);
	/**
	 *申请审批查询实体
	 * @param dwbh
	 * @return
	 */
	public Map<String, Object> getObjectById1(String guid) {
		String sql="(select a.guid, a.djbh,a.sqr,"
				+ "(select '('||r.rybh||')' || r.xm from gx_sys_ryb r where r.guid=a.sqr)as sqrmc, a.szbm,"
				+ "'(' || a.szbm || ')' ||  (select d.mc from gx_sys_dwb d where d.dwbh = a.szbm) as szbmmc,  a.shzt,"
				+ "(select d.mc from gx_sys_dmb d where d.zl = '11033' and d.dm=a.shzt)as shztmc,  to_char(a.czrq,'yyyy-mm-dd') as czrq, '出差业务'as sqsplx"
				+ " from CW_CCSQSPB A  "
				+ "LEFT JOIN CW_JFSZB B ON A.JFBH = B.GUID where a.guid='"+guid+"') union select a.guid, a.djbh, a.sqr, (select '('||r.rybh||')' || r.xm from gx_sys_ryb r where r.rybh=a.sqr)as sqrmc,"
				...
				+ " from CW_GWJDYWSQSPB a"
				+ " where guid= ?";
		return db.queryForMap(sql,new Object[]{guid});
	};
	
	
	public List<Map<String, Object>> getJsList(String sql,Object dwbh2) {
...
//		};
		 ...
		 */
//		public Map<String, Object> getObjectById1(String guid) {
//			String sql="select guid,wlbh,dwmc,dwjc,dwlx,dwdz,sh,lxr,bgdh,cz from cw_wldwb where guid= ?";
//			return db.queryForMap(sql,new Object[]{guid});
//		};
...
//		String sql = " select g.guid,g.bmbh,('('||g.bmbh||')'||(select d.mc from gx_sys_dmb d where d.dm = g.bmbh)) as bmmc,"
