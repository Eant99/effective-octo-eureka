 ...
 */
public class sqspcxExportExcel {
	 public static Map<String, Object> exportExcel(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
			...
			try {      
			   ...
			   for (Map<String, Object> drow : listContent) {
					...
					sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("SQRMC"), ""), wcf_left));
					sheet.addCell(new Label(4, i, Validate.isNullToDefaultString(drow.get("SZBMMC"), ""), wcf_left));
					sheet.addCell(new Label(5, i, Validate.isNullToDefaultString(drow.get("SQSPLX"), ""), wcf_left));
					sheet.addCell(new Label(6, i, Validate.isNullToDefaultString(drow.get("SQRQ"), ""), wcf_left));
					i++;
			    }
			   /************将以上缓存中的内容写到EXCEL文件中*********/  
			   workbook.write();  
