...
public class XwryxxcjExportExcl {
	...
	public static Map<String, Object> exportExcel(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
		...
		try {      
		   /** **********创建工作簿************ */  
			...
			System.err.println("1111!!!!!!!!!!!!!!!!!!!"+f.exists());
			WritableWorkbook workbook = Workbook.createWorkbook(f);
		   /** **********创建工作表************ */  
		   WritableSheet sheet = workbook.createSheet("校外人员细信息采集",0);
		   ...
		   for (Map<String, Object> drow : listContent) {
				...
				sheet.addCell(new Label(0, i, Validate.isNullToDefaultString(drow.get("xh"), ""), wcf_left));
				sheet.addCell(new Label(1, i, Validate.isNullToDefaultString(drow.get("xm"), ""), wcf_left));
				sheet.addCell(new Label(2, i, Validate.isNullToDefaultString(drow.get("xbm"), ""), wcf_left));
				sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("csrq"), ""), wcf_left));
				sheet.addCell(new Label(4, i, Validate.isNullToDefaultString(drow.get("SFZJLXM"), ""), wcf_left));
				sheet.addCell(new Label(5, i, Validate.isNullToDefaultString(drow.get("sfzh"), ""), wcf_left));
				sheet.addCell(new Label(6, i, Validate.isNullToDefaultString(drow.get("yhmc"), ""), wcf_left));
				sheet.addCell(new Label(7, i, Validate.isNullToDefaultString(drow.get("khyhh"), ""), wcf_left));
				sheet.addCell(new Label(8, i, Validate.isNullToDefaultString(drow.get("LHH"), ""), wcf_left));
				i++;
		    }
		   /************将以上缓存中的内容写到EXCEL文件中*********/  
		   workbook.write();  
