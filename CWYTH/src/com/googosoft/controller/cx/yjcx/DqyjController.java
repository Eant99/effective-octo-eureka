 ...
 */
@Controller
@RequestMapping(value="/dqyj")
public class DqyjController extends BaseController{
	 ...
	 * 获取资产到期预警列表页面
	 */
	@RequestMapping(value="/goDqyjPage")
	public ModelAndView goDqyjPage(){
		ModelAndView mv = this.getModelAndView();
		PageData pd = this.getPageData();
		String dwbh = pd.getString("dwbh");
		Map map = xtbService.getXtcs(); //只需要查询一条数据，不要根据guid 查询
	
		mv.addObject("xtb", map.get("YJTS"));
		...
		mv.setViewName("cx/yjcx/dqyj_list");
		return mv;
	}
	...
	public Object getPageList(){
		...
		sqltext.append(" DECODE(K.ZTBZ,'99','财务通过','') AS ZTBZ,nvl(K.SYKH,1)SYKH,K.YQBH AS YQBH,K.YQBH AS YQH,K.YQMC AS YQMC,K.FLH AS FLH,K.FLMC AS FLMC,K.FLGBM, ");
		...
		if(Validate.noNull(dwbh)){
			pageList.setStrWhere(" AND K.SYDW IN(SELECT DWBH FROM GX_SYS_DWB CONNECT BY PRIOR DWBH=SJDW START WITH DWBH='" + dwbh + "') AND K.ZTBZ = '"+StateManager.SHWC+"'  and   nvl(k.synx,'0') < (sysdate-k.rzrq)/365  and k.xz not in (select dm from gx_sys_dmb where zl='41')");
		}else{
			//当前登录人管理单位权限
			pageList.setStrWhere(pageService.getQxsql(userId, "k.SYDW", "D") + "AND K.ZTBZ = '"+StateManager.SHWC+"' and  nvl(k.synx,'0') < (sysdate-k.rzrq)/365  and k.xz not in (select dm from gx_sys_dmb where zl='41')");
		}
	...
	public Object ExpExcel(){
		...
		sqltext.append(" SELECT  DECODE(K.ZTBZ,'99','财务通过','') AS ZTBZ,nvl(K.SYKH,1)SYKH,K.YQBH AS YQBH,K.YQBH AS YQH,K.YQMC AS YQMC,K.FLH AS FLH,K.FLMC AS FLMC,K.FLGBM, ");
		sqltext.append(" decode(nvl(K.dj,0),0,'0.00',(to_char(round(k.dj,2),'fm99999999999990.00'))) as dj, ");
		sqltext.append(" decode(nvl(K.ZZJ,0),0,'0.00',(to_char(round(k.zzj,2),'fm99999999999990.00'))) as zzj, ");
		sqltext.append(" (SELECT MC FROM GX_SYS_DMB WHERE ZL = '"+Constant.SYFX+"'AND DM = K.SYFX ) AS syfx, ");
		sqltext.append(" (select '('||a.dwbh||')'||a.mc from GX_SYS_DWB a WHERE a.dwbh=K.sydw) as sydw,K.SYDW AS SYDWH,");
		...
		String userId = LUser.getRybh();
		if(Validate.noNull(dwbh)){
			sqltext.append(" AND K.SYDW IN(SELECT DWBH FROM GX_SYS_DWB CONNECT BY PRIOR DWBH=SJDW START WITH DWBH='" + dwbh + "') AND K.ZTBZ = '"+StateManager.SHWC+"'  and   nvl(k.synx,'0') < (sysdate-k.rzrq)/365  and k.xz not in (select dm from gx_sys_dmb where zl='41')");
		}else{
			//当前登录人管理单位权限
			sqltext.append(pageService.getQxsql(userId, "k.SYDW", "D") + "AND K.ZTBZ = '"+StateManager.SHWC+"' and  nvl(k.synx,'0') < (sysdate-k.rzrq)/365  and k.xz not in (select dm from gx_sys_dmb where zl='41')");
		}
