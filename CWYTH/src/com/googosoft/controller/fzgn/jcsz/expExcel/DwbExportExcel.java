 ...
 * 单位机构导出
 * @author wangzhiduo
 * @date 2017-12-28下午3:30:41
 */
public class DwbExportExcel {
	 public static Map<String, Object> exportExcel(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
			...
			try {      
			   ...
			   for (Map<String, Object> drow : listContent) {
					...
					sheet.addCell(new Label(1, i, Validate.isNullToDefaultString(drow.get("BMH"), ""), wcf_left));
					sheet.addCell(new Label(2, i, Validate.isNullToDefaultString(drow.get("MC"), ""), wcf_left));
					sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("JC"), ""), wcf_left));
					sheet.addCell(new Label(4, i, Validate.isNullToDefaultString(drow.get("DZ"), ""), wcf_left));
					sheet.addCell(new Label(5, i, Validate.isNullToDefaultString(drow.get("DWXZ"), ""), wcf_left));
					sheet.addCell(new Label(6, i, Validate.isNullToDefaultString(drow.get("JLRQ"), ""), wcf_left));
					sheet.addCell(new Label(7, i, Validate.isNullToDefaultString(drow.get("DWLD"), ""), wcf_left));
					sheet.addCell(new Label(8, i, Validate.isNullToDefaultString(drow.get("SJDW"), ""), wcf_left));
					sheet.addCell(new Label(9, i, Validate.isNullToDefaultString(drow.get("DWBB"), ""), wcf_left));
					sheet.addCell(new Label(10, i, Validate.isNullToDefaultString(drow.get("SFXY"), ""), wcf_left));
					sheet.addCell(new Label(11, i, Validate.isNullToDefaultString(drow.get("DWZT"), ""), wcf_left));
					i++;
			    }
			   /************将以上缓存中的内容写到EXCEL文件中*********/  
			   workbook.write();  
