...
public class XfxxExportExcel {
	/**
	 * 消费记录查询统计导出
	 * @author 作者：BiMing
	 * @version 创建时间:2018-4-14下午3:18:55
	 */
	public static Map<String, Object> exportExcel(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
		 ...
		 try {      
			 ...
			 for (Map<String, Object> drow : listContent) {
				 ...
				 sheet.addCell(new Label(1, i, Validate.isNullToDefaultString(drow.get("cbsmc"), ""), wcf_left));
				 sheet.addCell(new Label(2, i, Validate.isNullToDefaultString(drow.get("xfddmc"), ""), wcf_left));
				 sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("zlwzmc"), ""), wcf_left));
				 sheet.addCell(new Label(4, i, Validate.isNullToDefaultString(drow.get("ze"), ""), wcf_left));
				 i++;
			 }
			 /************将以上缓存中的内容写到EXCEL文件中*********/  
			 workbook.write();  
