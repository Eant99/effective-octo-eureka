 ...
 */
public class ccywsqExportExcel {
	 public static Map<String, Object> exportExcel(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
			...
			try {      
			   ...
			   for (Map<String, Object> drow : listContent) {
					...
					sheet.addCell(new Label(1, i, Validate.isNullToDefaultString(drow.get("SHZT"), ""), wcf_left));
					sheet.addCell(new Label(2, i, Validate.isNullToDefaultString(drow.get("DJBH"), ""), wcf_left));
					sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("SQRXM"), ""), wcf_left));
					sheet.addCell(new Label(4, i, Validate.isNullToDefaultString(drow.get("SZBM"), ""), wcf_left));
					sheet.addCell(new Label(5, i, Validate.isNullToDefaultString(drow.get("CCLX"), ""), wcf_left));
//					sheet.addCell(new Label(6, i, Validate.isNullToDefaultString(drow.get("JFLX"), ""), wcf_left));
					sheet.addCell(new Label(6, i, Validate.isNullToDefaultString(drow.get("KTMC"), ""), wcf_left));
					sheet.addCell(new Label(7, i, Validate.isNullToDefaultString(drow.get("CCTS"), ""), wcf_left));
					sheet.addCell(new Label(8, i, Validate.isNullToDefaultString(drow.get("SQRQ"), ""), wcf_left));
					i++;
			    }
			   /************将以上缓存中的内容写到EXCEL文件中*********/  
			   workbook.write();  
