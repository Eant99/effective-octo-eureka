...
import com.googosoft.service.kjhs.pzxx.PzdyService;
 ...
 */
@Controller
@RequestMapping("kjhs/pzxx/pzdy")
public class PzdyController extends BaseController{
	
	@Resource(name="pzdyService")
	PzdyService pzdyService;
	...
	public String iniPzInfo(ModelAndView mv,PageData pd,HttpSession session) {
		...
		pzlx=Validate.isNullToDefaultString(pd.getString("pzlx"), pzdyService.getPzlx(pznd,kjqj,sszt));
		//查询数据库中已经存在的凭证编号，没有会计制度的限制
		List<String> pzbhList = pzdyService.getPzbhList(pznd,kjqj,sszt,pzlx);
		//查询凭证类型列表
		...
		if("change".equals(pd.getString("qcz"))||Validate.isNull(pzbh)||"lf".equals(lf)){
			pzbh=pzdyService.getminPzbh(sszt, pznd, kjqj, pzlx);
		}
		String  minpzbh=pzdyService.getminPzbh(sszt, pznd, kjqj, pzlx);
		...
		if(pzbhList.contains(pzbh)) {
			Map<String, Object> pzzbMap = pzdyService.getPzlrMain(pznd,kjqj, pzbh,sszt,pzlx);
			mv.addObject("zbMap",pzzbMap);
			session.setAttribute("pzzbGuid", pzzbMap.get("guid"));
			mv.addObject("mxList",pzdyService.getPzlrDetail(pznd, kjqj, pzbh, sszt,pzlx));
			//凭证状态
	...
	public ModelAndView pageSkip(HttpSession session) {
		...
		mv.setViewName("kjhs/pzxx/pzdy/"+pageName);
		return mv;
	}
	//获取列表数据
	...
	public Object getPzJsonByBx() {
		...
		String type = pd.getString("type");
		List<Map<String, Object>> list = pzdyService.getPzJsonByBx(type,guid);
		Gson gson = new Gson();
	...
	public Object getEchoJson() {
		PageData pd = this.getPageData();
		Map<String, Object> map = pzdyService.getEchoData(pd);
		Gson gson = new Gson();
	...
	public Object getSuggestInfo() {
		PageData pd = this.getPageData();
		return pzdyService.getSuggestInfo(pd);
	}
	/**
	 *  复核
	 * @return
	...
	public Object Saverq() {
		...
		int result = 0;
		result =  pzdyService.updateFh(guid,pzzt,fhr);
		Gson gson = new Gson();
		if(result>0) {
	...
	public Object Saverqqx() {
		...
		String fhr = LUser.getGuid();
		int result = 0;
		result =  pzdyService.updateFhqx(guid,pzzt,fhr);
		Gson gson = new Gson();
		if(result>0) {
			return gson.toJson(true);
		}
	...
	public Object qxfh(HttpServletRequest req,HttpServletResponse res) {
		...
		int result = 0;
		result =  pzdyService.qxfh(guid,pzzt,fhyj);
		Gson gson = new Gson();
		if(result>0) {
			return gson.toJson(true);
		}
	...
	public Object plfh(HttpServletRequest req,HttpServletResponse res) {
		...
		if("zdrwk".equals(type)) {
			int result = 0;
			result =  pzdyService.plfh(guid,pzzt,qspdh,jspdh);
			if(result>0) {
				return gson.toJson(true);
			}
		}else if("zdr".equals(type)) {
			int result = 0;
			result =  pzdyService.plfhzdr(guid,pzzt,qspdh,jspdh,zdr);
			if(result>0) {
				return gson.toJson(true);
			}
		}
	...
	public Object plfhrq(HttpServletRequest req,HttpServletResponse res) {
		...
		if("zdrwk".equals(type)) {
			int result = 0;
			result =  pzdyService.plfhrq(guid,pzzt,qsrq,jsrq);
			if(result>0) {
				return gson.toJson(true);
			}
		}else if("zdr".equals(type)) {
			int result = 0;
			result =  pzdyService.plfhrqzdr(guid,pzzt,qsrq,jsrq,zdr);
			if(result>0) {
				return gson.toJson(true);
			}
		}
	...
	public Object pzdy(HttpSession session) {
		...
		List<Map<String, Object>> zbList = pzdyService.getPzlrMain(qspdh,jspdh,qsrq,jsrq,zdr,"01','02','99",Constant.getztid(session), CommonUtil.getKjzd(session)) ;
		...
		mv.setViewName("kjhs/pzxx/pzdy/print");
		return mv;
	}
	...
	public ModelAndView goplfhWindowPage(HttpSession session){
		...
		String sszt = Constant.getztid(session);
		List zbList = pzdyService.getPzlrZbList("00",pzz,sszt);
		List zdrList = pzdyService.getZdrList("00",sszt);
		mv.addObject("guid",guid);
	...
	public ModelAndView goPrintwindowPage(HttpSession session){
		...
		String sszt = Constant.getztid(session);
		List zbList = pzdyService.getPzlrZbList("01','02','99",pzz,sszt);
		List zdrList = pzdyService.getZdrList("01','02','99",sszt);
		mv.addObject("zbList",zbList);
		mv.addObject("zdrList",zdrList);
		mv.addObject("ryxm",LUser.getRyxm());
		mv.setViewName("kjhs/pzxx/pzdy/pzdyWindow");
		return mv;
	}
