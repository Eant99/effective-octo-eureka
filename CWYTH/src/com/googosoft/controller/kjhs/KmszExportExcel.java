 ...
 * 科目余额初始化导出
 * @author wangzhiduo
 * @date 2017-12-28下午7:17:12
 */
public class KmszExportExcel {
	 public static Map<String, Object> exportExcel(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
			...
			try {      
			   ...
			   for (Map<String, Object> drow : listContent) {
					...
					sheet.addCell(new Label(1, i, Validate.isNullToDefaultString(drow.get("KMBH"), ""), wcf_left));
					sheet.addCell(new Label(2, i, Validate.isNullToDefaultString(drow.get("KMMC"), ""), wcf_left));
					sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("YEFX"), ""), wcf_left));
					sheet.addCell(new Label(4, i, Validate.isNullToDefaultString(drow.get("KMZYE"), ""), wcf_left));
					i++;
			    }
			   /************将以上缓存中的内容写到EXCEL文件中*********/  
			   workbook.write();  
