...
public class PjytExportExcel {
	...
	System.err.println("1111!!!!!!!!!!!!!!!!!!!"+f.exists());
			   ...
			   /** **********创建工作表************ */  
			   WritableSheet sheet = workbook.createSheet("票据用途信息",0);
			   ...
			   for (Map<String, Object> drow : listContent) {
					...
					sheet.addCell(new Label(1, i, Validate.isNullToDefaultString(drow.get("pjytbh"), ""), wcf_left));
					sheet.addCell(new Label(2, i, Validate.isNullToDefaultString(drow.get("pjytmc"), ""), wcf_left));
					sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("SFQYMC"), ""), wcf_left));
					sheet.addCell(new Label(4, i, Validate.isNullToDefaultString(drow.get("BZ"), ""), wcf_left));
					i++;
			    }
			   /************将以上缓存中的内容写到EXCEL文件中*********/  
			   workbook.write();  
	 ...
	 public static Map<String, Object> exportExcel2(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
			...
			try {      
			   ...
			   /** **********创建工作表************ */  
			   WritableSheet sheet = workbook.createSheet("票据账户信息",0);
			   sheet.getSettings().setVerticalFreeze(2);
			   for(int i=1;i<title.length;i++){
				   sheet.setColumnView(i,30); 
			   }
			   ...
			   for (Map<String, Object> drow : listContent) {
				    sheet.setRowView(i, 300, false); 
					sheet.addCell(new Label(0, i, Validate.isNullToDefaultString(i-1, ""), wcf_left));
					sheet.addCell(new Label(1, i, Validate.isNullToDefaultString(drow.get("zhmc"), ""), wcf_left));
					sheet.addCell(new Label(2, i, Validate.isNullToDefaultString(drow.get("pjlxmc"), ""), wcf_left));
					sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("sfqymc"), ""), wcf_left));
					i++;
			    }
			   /************将以上缓存中的内容写到EXCEL文件中*********/  
