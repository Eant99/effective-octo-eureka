	 ...
	 public static Map<String, Object> exportExcel(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
			...
			try {      
			   ...
			   for (Map<String, Object> drow : listContent) {
				    ...
				    String zrye = drow.get("ZRYE")+"";
				    String jrfsje = drow.get("JRFSJE")+"";
				    String jrfsdf = drow.get("JRFSDF")+"";
				    String jrye = drow.get("JRYE")+"";
...
//					sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("ZRYE"), ""), wcf_left));
//					sheet.addCell(new Label(4, i, Validate.isNullToDefaultString(drow.get("JRFSJE"), ""), wcf_left));
//					sheet.addCell(new Label(5, i, Validate.isNullToDefaultString(drow.get("JRFSDF"), ""), wcf_left));
					if(Validate.noNull(zrye)) {
						double bd =	Double.parseDouble(zrye.replace(",", ""));
						jxl.write.Number labelNF = new jxl.write.Number(3,i,bd,wcf_left);
						sheet.addCell(labelNF);
					}else {
						sheet.addCell(new Label(3, i, "", wcf_left));
					}
					if(Validate.noNull(jrfsje)) {
						double bd =	Double.parseDouble(jrfsje.replace(",", ""));
						jxl.write.Number labelNF = new jxl.write.Number(4,i,bd,wcf_left);
						sheet.addCell(labelNF);
					}else {
						sheet.addCell(new Label(4, i, "", wcf_left));
					}
					if(Validate.noNull(jrfsdf)) {
						double bd =	Double.parseDouble(jrfsdf.replace(",", ""));
					...
					//sheet.addCell(new Label(7, i, Validate.isNullToDefaultString(drow.get("JRYE"), ""), wcf_left));
					if(Validate.noNull(jrye)) {
						double bd =	Double.parseDouble(jrye.replace(",", ""));
						jxl.write.Number labelNF = new jxl.write.Number(7,i,bd,wcf_left);
						sheet.addCell(labelNF);
					}else {
						sheet.addCell(new Label(7, i, "", wcf_left));
					}
					sheet.addCell(new Label(8, i, Validate.isNullToDefaultString(drow.get("JFBS"), ""), wcf_left));
					sheet.addCell(new Label(9, i, Validate.isNullToDefaultString(drow.get("DFBS"), ""), wcf_left));
					i++;
			    }
			   /************将以上缓存中的内容写到EXCEL文件中*********/  
			   workbook.write();  
