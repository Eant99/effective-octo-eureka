	 ...
	 public static Map<String, Object> exportExcel(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
			...
			try {      
			   ...
			   for (Map<String, Object> drow : listContent) {
					...
					sheet.addCell(new Label(4, i, Validate.isNullToDefaultString(drow.get("JJFL"), ""), wcf_left));
					sheet.addCell(new Label(5, i, Validate.isNullToDefaultString(drow.get("BMBH"), ""), wcf_left));
					sheet.addCell(new Label(6, i, Validate.isNullToDefaultString(drow.get("XMBH"), ""), wcf_left));
//					sheet.addCell(new Label(7, i, Validate.isNullToDefaultString(drow.get("JFJE"), ""), wcf_left));
					...
					if(Validate.noNull(dfje)) {
						...
						jxl.write.Number labelNF = new jxl.write.Number(8,i,bd,wcf_left);
						sheet.addCell(labelNF);
					}else {
						sheet.addCell(new Label(8, i, "", wcf_left));
					}
					sheet.addCell(new Label(9, i, Validate.isNullToDefaultString(drow.get("FX"), ""), wcf_left));
					if(Validate.noNull(ye)) {
						double bd =	Double.parseDouble(ye.replace(",", ""));
						sheet.addCell(new jxl.write.Number(10, i, bd, wcf_left));
					}else{
