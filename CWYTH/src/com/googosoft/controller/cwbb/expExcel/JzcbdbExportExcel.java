 ...
 * 净资产变动导出
 * @author wangzhiduo
 * @date 2017-12-29上午8:41:11
 */
public class JzcbdbExportExcel {
	 public static Map<String, Object> exportExcel(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
			...
			try {      
			   ...
			   for (Map<String, Object> drow : listContent) {
					...
					sheet.addCell(new Label(2, i, Validate.isNullToDefaultString(drow.get("BNLJYY"), ""), wcf_left));
					sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("BNZYJJ"), ""), wcf_left));
					sheet.addCell(new Label(4, i, Validate.isNullToDefaultString(drow.get("BNQYFTZ"), ""), wcf_left));
					sheet.addCell(new Label(5, i, Validate.isNullToDefaultString(drow.get("BNJZCHJ"), ""), wcf_left));
					sheet.addCell(new Label(6, i, Validate.isNullToDefaultString(drow.get("SNLJYY"), ""), wcf_left));
					sheet.addCell(new Label(7, i, Validate.isNullToDefaultString(drow.get("SNZYJJ"), ""), wcf_left));
					sheet.addCell(new Label(8, i, Validate.isNullToDefaultString(drow.get("SNQYFTZ"), ""), wcf_left));
					sheet.addCell(new Label(9, i, Validate.isNullToDefaultString(drow.get("SNJZCHJ"), ""), wcf_left));
					i++;
			    }
			   /************将以上缓存中的内容写到EXCEL文件中*********/  
			   workbook.write();  
