 ...
 * 财政补助收入支出表导出
 * @author wangzhiduo
 * @date 2018-1-4下午2:23:59
 */
public class CzbzsrzcExcel {
	 public static Map<String, Object> exportExcel(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
			...
			try {      
			   ...
			   for (Map<String, Object> drow : listContent) {
				    ...
				    String bys = drow.get("BYS")+"";
				    String bnljs = drow.get("BNLJS")+"";
...
//					sheet.addCell(new Label(2, i, Validate.isNullToDefaultString(drow.get("BYS"), ""), wcf_left));
//					sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("BNLJS"), ""), wcf_left));
					if(Validate.noNull(bys)) {
						double bd =	Double.parseDouble(bys.replace(",", ""));
						jxl.write.Number labelNF = new jxl.write.Number(2,i,bd,wcf_left);
						sheet.addCell(labelNF);
					}else {
						sheet.addCell(new Label(2, i, "", wcf_left));
					}
					if(Validate.noNull(bnljs)) {
						double bd =	Double.parseDouble(bnljs.replace(",", ""));
