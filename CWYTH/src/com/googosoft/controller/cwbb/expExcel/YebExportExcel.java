...
@Repository("yebExportExcel")
public class YebExportExcel {
	public Map<String, Object> ZjszExportExcel(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
		...
		try {      
		   ...
		   /** **********创建工作表************ */  
		   WritableSheet sheet = workbook.createSheet("资金收支余额表",0);
		   ...
		   for (Map<String, Object> drow : listContent) {
				...
				sheet.addCell(new Label(1, i, Validate.isNullToDefaultString(drow.get("XMMC").toString().replace("<b>", "").replace("</b>", ""), ""), wcf_left));
				sheet.addCell(new Label(2, i, Validate.isNullToDefaultString(drow.get("YE"), ""), wcf_left));
				i++;
		    }
		   /************将以上缓存中的内容写到EXCEL文件中*********/  
		   workbook.write();  
