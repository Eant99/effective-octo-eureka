...
public class ZcfzExportExcel {
	 public static Map<String, Object> exportExcel(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
			...
			try {      
			   ...
			   for (Map<String, Object> drow : listContent) {
					...
					sheet.addCell(new Label(1, i, Validate.isNullToDefaultString(drow.get("ZC"), ""), wcf_left));
					sheet.addCell(new Label(2, i, Validate.isNullToDefaultString(drow.get("QMYE1"), ""), wcf_left));
					sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("NCYE1"), ""), wcf_left));
					sheet.addCell(new Label(4, i, Validate.isNullToDefaultString(drow.get("FZHJZC"), ""), wcf_left));
					sheet.addCell(new Label(5, i, Validate.isNullToDefaultString(drow.get("QMYE2"), ""), wcf_left));
					sheet.addCell(new Label(6, i, Validate.isNullToDefaultString(drow.get("NCYE2"), ""), wcf_left));
					i++;
			    }
			   /************将以上缓存中的内容写到EXCEL文件中*********/  
			   workbook.write();  
	 ...
	 //导出新的资产负债表
		 public static Map<String, Object> exportExcel2(String fileurl,String shortfileurl,String[] title,String TITLE, List<Map<String, Object>> listContent) {  
				String result="系统提示：Excel文件导出成功！"; 
				boolean flag=true;
				//以下开始输出到EXCEL  
				try {      
				   ...
				   for (Map<String, Object> drow : listContent) {
					    sheet.setRowView(i, 300, false); 
					    String qmye1 = drow.get("QMYE1")+"";
					    String ncye1 = drow.get("NCYE1")+"";
					    String qmye2 = drow.get("QMYE2")+"";
					    String ncye2 = drow.get("NCYE2")+"";
						sheet.addCell(new Label(0, i, Validate.isNullToDefaultString(i-1, ""), wcf_left));
						sheet.addCell(new Label(1, i, Validate.isNullToDefaultString(drow.get("ZC"), "").replace("<b>", "").replace("</b>", ""), wcf_left));
//						sheet.addCell(new Label(2, i, Validate.isNullToDefaultString(drow.get("QMYE1"), ""), wcf_left));
//						sheet.addCell(new Label(3, i, Validate.isNullToDefaultString(drow.get("NCYE1"), ""), wcf_left));
						if(Validate.noNull(qmye1)) {
							double bd =	Double.parseDouble(qmye1.replace(",", ""));
							jxl.write.Number labelNF = new jxl.write.Number(2,i,bd,wcf_left);
							sheet.addCell(labelNF);
						}else {
							sheet.addCell(new Label(2, i, "", wcf_left));
						}
						if(Validate.noNull(ncye1)) {
							double bd =	Double.parseDouble(ncye1.replace(",", ""));
							jxl.write.Number labelNF = new jxl.write.Number(3,i,bd,wcf_left);
							sheet.addCell(labelNF);
						}else {
							sheet.addCell(new Label(3, i, "", wcf_left));
						}
						sheet.addCell(new Label(4, i, Validate.isNullToDefaultString(drow.get("FZHJZC"), "").replace("<b>", "").replace("</b>", ""), wcf_left));
//						sheet.addCell(new Label(5, i, Validate.isNullToDefaultString(drow.get("QMYE2"), ""), wcf_left));
//						sheet.addCell(new Label(6, i, Validate.isNullToDefaultString(drow.get("NCYE2"), ""), wcf_left));
						if(Validate.noNull(qmye2)) {
							double bd =	Double.parseDouble(qmye2.replace(",", ""));
							jxl.write.Number labelNF = new jxl.write.Number(5,i,bd,wcf_left);
							sheet.addCell(labelNF);
						}else {
							sheet.addCell(new Label(5, i, "", wcf_left));
						}
						if(Validate.noNull(ncye2)) {
							double bd =	Double.parseDouble(ncye2.replace(",", ""));
							jxl.write.Number labelNF = new jxl.write.Number(6,i,bd,wcf_left);
							sheet.addCell(labelNF);
						}else {
