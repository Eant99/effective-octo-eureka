...
public class ThreeDESUtil3 {

	// 定义加密算法，有DES、DESede(即3DES)、Blowfish
	private static final String Algorithm = "DESede";
	private static final String PASSWORD_CRYPT_KEY = "9021270501180312";
	 ...
	 * @return
	 */
	public static String encryptMode(byte[] src,byte[] code) {
		try {
			...
			System.out.println();
//			System.out.println(new String(c1.doFinal(src)));
//			String baseString = Base64.encode(c1.doFinal(src)); XkTTBmRouGo= XkTTBmRouGo=
			sun.misc.BASE64Encoder encoder = new sun.misc.BASE64Encoder();   
			byte[] result = c1.doFinal(src);
//			System.out.println("长度++++++++++++++++++"+result.length);
			String baseString = encoder.encode(result);
			return baseString;
	 ...
	 * @return
	 */
	public static byte[] decryptMode(byte[] src,byte[] desCode) {
		try {
			SecretKey deskey = new SecretKeySpec(
					desCode, Algorithm);
	...
	public static void main(String[] args) {
		...
		// 加密
//		byte[] secretArr = ThreeDESUtil3.encryptMode(msg.getBytes());
//		System.out.println("【加密后】：" + new String(secretArr));

		// 解密
//		byte[] myMsgArr = ThreeDESUtil3.decryptMode(secretArr);
